...
Never hardcode the brand (`Dam`, `dam`, or any replacement) in code. The codename `platform` is permanent; user-visible brand flows through the node configuration (`BRAND_*`, written from Helm `brand.*` ([`helm/values.yaml`](helm/values.yaml)) on cluster-provisioned nodes) → api-server `config.brand` → UI `getBrand()` ([`packages/ui/src/brand.ts`](packages/ui/src/brand.ts)).
