...
operational lessons. Verified environment facts and recurring failure modes ("this API
paginates at 100"), written **only when a root cause was actually reproduced**, read at
the start of work runs. It prevents every future run from re-diagnosing the same
environmental quirk; entries name the evidence.
