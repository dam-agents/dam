...
- **Size.** Per-argument length and argv count are capped, before any matching begins. A user-authored regex meeting model-supplied input is a ReDoS on exactly the machine this feature exists to protect, and the cap alone does not close it: a legal argument still meets a pattern that backtracks. So the regexes run on a worker thread under a deadline and the api-server thread never evaluates one. The thread is started once and reused, because starting one costs more than the deadline it enforces; it receives the distinct sources and the arguments as two lists rather than every pair, compiles each source once, and is abandoned and replaced when a command overruns. What stays on the request thread is bounded on its own terms: the matcher carries a step budget shared across a Manifest's patterns, and a Manifest may declare only so many pattern tokens.
...
- **Authentication** uses the existing bearer or API key ([cli](../architecture/cli.md#authentication)) under a new `satellites:serve` scope, so a leaked Satellite key can serve a Satellite and cannot operate Agents. It must be an unrestricted principal: a Satellite serves every Agent granted to it, so binding the key to one Agent would not stop it claiming another's work, and pretending otherwise is worse than saying so.
...
Running Jobs keep executing under the Command Pattern they matched at start: re-validating them against a new Manifest would be retroactive policy, and the process is running regardless. **A reload is refused for three reasons, and each keeps the running Manifest** — it does not parse, which would otherwise drop the machine to an empty command set on a typo; it renames the Satellite, which is a different machine under `(owner, name)` and needs a restart; or the machine is draining, where widening what may run is the wrong answer. A reload whose push to the platform fails keeps the running Manifest too. An accepted reload reaches the Snapshot before the worker, so the machine never enforces a Manifest the server has not taken. A reload that *removes* a pattern takes effect at once for new starts while the Agent's tool description still advertises it, which is the discovery lag arriving by a new door.
