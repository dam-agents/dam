...
- **One outbox surface — snapshot-shaped.** A `runtime_state_outbox` row exists at most one per agent. It carries no payload, only the marker that "this agent's desired delivery changed; recompute and push." Events are not a second outbox: they live in `runtime_events` (one row per pending event, with `dispatched_at` and `expires_at`), and the state-builder reads non-dispatched events as part of constructing the payload. Any change to either contributions or events bumps the agent's single outbox row.

- **Mutation path is one Postgres transaction plus one BullMQ enqueue.** The handler commits the domain change (contribution edit or event insert) and the outbox row atomically, then enqueues a BullMQ job after commit. The user-facing response returns immediately; agent reachability does not influence response time.
...
- **Events commit at their own RPC, not at the worker's apply-ack.** When the agent processes an event from the payload, it calls `executeEvent` (e.g. `sessions.create(triggerId, …)` for a `trigger` event) on the harness API server. That harness handler is idempotent on the event's stable id — a unique constraint on the side-effect table (e.g. `sessions.trigger_event_id`) ensures a second call with the same id finds the existing row and returns it without redoing the work. Inside the same transaction, the handler stamps `runtime_events.dispatched_at = now()`. The worker does not stamp anything on event rows; the harness handler does.
...
- **`hello` reads the same state-builder.** A waking agent calls `hello`; the harness server runs the same state-builder, compares the payload's `version` and `hash` against what the agent reports, and returns the new payload only if anything diverged. Events the agent receives via `hello` follow the same execute-and-commit rule.
...
- **Worker stamps `dispatched_at` on event rows after the `applyState` ack** — rejected. Earlier draft. Required the worker to hold the included event ids in memory across the round-trip and `UPDATE … WHERE id IN (...)` after success, plus an agent-side PVC log to dedupe across "agent crashed between event-fire and apply-ack" cases. The cleaner shape — server-side handler idempotency on the event id — pushes the dedupe to a single SQL constraint in the harness, eliminates the agent-side log entirely, and aligns commit with the side effect (the `sessions.create` row IS the dispatch ack, not a separate stamp).

- **Two outbox surfaces — `runtime_state_outbox` and `runtime_signal_outbox`** — rejected. An even earlier draft. Modeled triggers as discrete signal events with per-event TTL, per-event retry budget, and per-event ack against a dedicated signal queue. Once ADR-048 collapsed signals into the runtime payload, the second outbox became a delivery rail without distinct delivery semantics. One outbox, one queue, one set of operational metrics.
...
- **Easier:** No agent-side persistent dedupe needed. The `runtime_events.dispatched_at` stamp lives next to the side-effect row (sessions row for triggers); a unique constraint at the harness handler makes redelivery idempotent. The worker doesn't track per-event state across the apply round-trip.

- **Easier:** One queue, one worker, one set of metrics. The earlier signal-channel and worker-stamp designs each added a parallel surface; collapsing eliminates both.
...
- **Harder:** Each event kind's harness handler needs an idempotency key (a unique constraint joining its side-effect table back to `runtime_events.id`). For triggers that's a new `trigger_event_id` column on `sessions`; future kinds inherit the same pattern. The pattern is small and uniform but must be applied consistently or a kind's redelivery becomes lossy or doubling-firing.
