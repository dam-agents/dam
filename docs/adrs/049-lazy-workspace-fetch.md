...
The set of expanded directories is per-agent and held in client memory for the lifetime of the page session. Server-reported missing paths are silently dropped from the expanded set. Mutation handlers invalidate the whole batched query rather than splicing per-directory results.
