...
- **`events`** — an ordered stream of one-shot directives the agent must execute (trigger fires today; rotate / rescan / … in the future). Processed in order. Each event commits at its own per-kind handler on the harness API server; the handler is idempotent on the event's stable id.

- **Two routes**, prefixed by protocol-major version. The api-server calls *into* the agent's harness port with `applyState` (push current `version`, `state`, and currently-pending `events` for one agent). The agent calls *into* the api-server's harness-API-server port with `hello` (boot/wake catch-up — returns the same envelope if anything diverged). When the agent processes an event, it calls back into the harness API's per-kind event handler — what that endpoint is and how it does its work is an implementation detail per event kind. No persistent WebSocket; every interaction is a tRPC round-trip whose response carries its own ack.
...
- **Event semantics.** Each event is `{ id, kind, payload, version, expiresAt }`. The agent processes events in order. Each event handler calls a per-kind endpoint on the harness API server; the harness handler is idempotent on the event's stable id (a unique constraint on its side-effect table) — a second call with the same id finds the existing side-effect row and returns it without redoing the work. The worker is the only writer to `runtime_events.dispatched_at`; the work-doing handler does not touch the outbox.

- **At-most-once with at-least-once delivery.** If the agent crashes after invoking a work-doing handler but before sending the `applyState` ack, the worker re-sends on the next dispatch. The agent retries the handler; the uniqueness constraint catches the duplicate; the second call returns the existing side-effect row; the next ack stamps `dispatched_at`. Combined: at-least-once delivery + handler idempotency = at-most-once side effects, no agent-side persistent state needed.
...
- **Per-event ack via a response field (`processedEventIds`)** — rejected. Carrying per-event acks in the apply response splits the "is this event done" fact into two writes (the side-effect commit at the work-doing handler, plus the apply-response handler's stamp), which can disagree under partial-failure modes. The cursor model collapses to one write: the worker stamps `dispatched_at` for events with version ≤ acked. The work-doing handler does not touch the outbox, and idempotency lives on its own side-effect table's uniqueness constraint.
...
- **Agent-side PVC dedupe log instead of work-handler idempotency** — rejected. The work-doing handler already needs to know the event's stable id to look up context; making it idempotent on that id (e.g., `INSERT … ON CONFLICT (event_id) DO NOTHING RETURNING …`) is a one-statement change in the relevant module. Moving the dedupe to a file on the agent PVC adds a write the agent must `fsync` before returning, a JSON file to GC, and a forks-don't-have-this carve-out.
...
- **Easier:** Single cursor for both slices. State monotonicity defense and event-done bookkeeping share one ack marker, one server-side write.

- **Easier:** No agent-side persistent dedupe. Server-side handler idempotency (a unique constraint joining its side-effect table to `runtime_events.id`) handles the crash-during-fire case cleanly. The agent's event handler is a plain RPC.
...
- **Harder:** Each event kind's harness handler must be idempotent on the event id, via a unique constraint joining its side-effect table back to `runtime_events.id`. The pattern is small and uniform but must be applied consistently or a kind's redelivery becomes lossy or double-firing.
