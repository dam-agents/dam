...
- **Driver order is manifest order.** The dispatcher applies kind-drivers in the order their kinds appear in the agent image's `runtime-manifest.yaml` `drivers` map (`Object.entries` insertion order). This is the only ordering lever; producers do not order contributions. `env` is not a runtime-channel driver — it is controller-rendered into the pod at start, so it is always in place before any driver runs.

- **`workspace-git` Contribution kind — one-shot working-dir seed.** Clones a public git repo into the agent's working directory (`/home/agent/work`) once, shortly after the agent is Ready. It is a state Contribution (so it can be ordered before `skill-ref` — events run *after* state and could not), but it is **one-shot and additive/non-removable**, unlike reconciled kinds: the driver clones only into an empty work dir, **skips** when `<work>/.git` already exists, and **fails** rather than clone into a non-empty dir; "removal" (deselecting the seed) is a no-op — the platform never un-seeds a working tree it may have handed to the user. Bound **first** in `runtime-manifest.yaml` so the workspace exists before skills/files layer onto it. The repo URL is chosen at create time from the operator-curated `gitRepos` catalog. Adding the kind is additive + capability-gated (no protocol bump); agent images that predate the `workspace-clone` driver binding simply don't advertise it and skip the seed.
