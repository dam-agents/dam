...
The snapshot is **computed in the knowledge base's own agent pod** — both the shared file objects and the search index — and written to object storage by the agent through **size-capped presigned upload URLs scoped to that share's key namespace**, the way artifacts are uploaded. The api-server only mints those URLs and atomically validates and commits the resulting manifest; it does not read the workspace or build the index on its own event loop.

- Publishing is **incremental**: the agent diffs its current workspace against the previous snapshot by content hash and re-reads, re-indexes, and re-uploads only what changed; unchanged file objects carry forward by content-addressed key. Change *detection* is a hashing walk today and can later be driven by a filesystem watch without a format change.
...
- **Committed-to:** the object store is on the consumer read path and is written directly by agents (the artifacts precedent), with the api-server as the sole manifest authority and presigned-URL scoping plus size caps as the only bound on what an agent can write for a share; the in-cluster harness route carries a second consumer besides platform-outbound.
