...
2. Resolves the calling pod's instance ID from the **TCP source IP** of the gRPC connection, not from any metadata header. A NetworkPolicy on the api-server restricts ingress on this port to pods carrying `humr.ai/instance` in the agent namespace, and the kernel/CNI prevents source-IP spoofing under the agent's pod security context (no `CAP_NET_RAW`, no `hostNetwork`). Concretely: the api-server keeps a `podIP → instanceId` cache rebuilt from a periodic `LIST pods` (10 s by default) plus an on-miss refresh, so a Check call from a known agent IP resolves synchronously. A compromised agent that bypasses its sidecar still resolves to *its own* instance ID — it can't impersonate a sibling. `initial_metadata` is no longer trusted; the controller may render it for log diagnostics but the gate ignores it.
