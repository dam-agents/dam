# ADR-055: Browser-driven E2E tracer with values-gated test affordances
...
Test-only artifacts ship in the production Helm chart behind `e2e.*` values flags, default off: a pre-seeded Keycloak user and a mock Agent Template (`packages/agents/mock/`). The mock agent is controllable at runtime via a test-only tRPC namespace on the api-server (gated by the same `e2e.enabled` flag) that proxies to the per-Agent mock pod. The surface covers ACP-frame-level scripting (set the sequence of frames the mock emits in response to prompts) and inbound observation (read back the frames the mock received from agent-runtime). The test cluster install flips the flags on; prod installs never see the affordances or the tRPC namespace.
...
- **Per-pod mock control plane (own Service + Ingress per Agent)** — cleanest separation but per-Agent ingress entries are fiddly; api-server already addresses agent pods for the ACP relay, so a test-only tRPC namespace there reuses the addressing.
...
- **Harder:** the production chart now carries test-only resources behind values flags, and api-server now carries a test-only tRPC namespace conditionally registered on `e2e.enabled`. Reviewers have to remember both exist and that flipping defaults on would surface a fake user, a mock Template, and the control namespace in real installs.
