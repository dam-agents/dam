...
**Agent name and avatar on replies:** with the optional `chat:write.customize` scope, agent replies post under the agent's name instead of the app's. For the agent's avatar to show too, set `--set=apiServer.imgbbApiKey=<key>` ([ImgBB API](https://api.imgbb.com)): Slack fetches message icons from the public internet, so the api-server uploads each avatar there as a PNG, named by a hash. Those images are public to anyone holding the URL.
