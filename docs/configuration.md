...
Templates with `backend.type: vm` run as smolvm microVMs inside the **VM runner** — one chart-rendered pod (`<release>-vm-runner`) built from `packages/controller/Dockerfile.vm-runner` that holds `/dev/kvm`. Enable it with `virtualization.enabled=true`; the pod requests KVM and tun as device-plugin resources, runs as root with NET_ADMIN, and keeps every machine's disk on one PVC (`virtualization.node.storage`). Any node with `/dev/kvm` will do: `virtualization.devicePlugin.enabled=true` ships a small DaemonSet that advertises the devices on the nodes its `nodeSelector`/`tolerations` pick (give the runner pod the same placement); without it the pod asks for KubeVirt's `devices.kubevirt.io/kvm` and `/tun` and lands on a virt node. On OpenShift bind an SCC that admits root + NET_ADMIN for the runner and the privileged plugin (`virtualization.node.scc: privileged` and `virtualization.devicePlugin.scc: privileged` render the RoleBindings). A NetworkPolicy limits the machine API and the published guest ports to the api-server and the controller; `virtualization.node.ingressCidrs` adds sources the runner itself admits on top. Where no device plugin exists, `virtualization.node.privileged: true` is the fallback, at the cost of the runner chmod-ing the host's `/dev/kvm` and `/dev/net/tun` world-writable. The runner pulls agent images from their registry (private registries need credentials in smolvm's own settings inside the pod; agent pull secrets are not forwarded yet). Locally, `mise run cluster:install -- --set=virtualization.enabled=true` creates the k3s Lima VM with nested virtualization (Apple silicon M3+ on macOS 15+), runs the same image with the same device plugin, and copies the locally built vm image archive into the pod since there is no registry. Nested virtualization is create-time only — an existing VM needs `cluster:delete` and a reinstall to gain it.
