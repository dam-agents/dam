...
Templates with `backend.type: vm` run as smolvm microVMs on a **sandbox node** — a Linux machine with KVM outside the cluster that the chart provisions over SSH. Enable it with `virtualization.enabled=true` and point the chart at the machine: `virtualization.node.host`, `virtualization.node.sshUser` (passwordless sudo) and `--set-file virtualization.node.sshKey=<private key>`. A post-install/upgrade Job then installs the pinned smolvm release and the `sandbox-node` agent, mints and places the API credentials, and routes the cluster's Service CIDR through a Kubernetes node (discovered from the ServiceCIDR API; `virtualization.node.serviceCidr` overrides). The node needs outbound internet for the smolvm download and to pull agent images from their registry (private registries need credentials in smolvm's own settings on the node; agent pull secrets are not forwarded yet). Locally, `mise run cluster:install -- --set=virtualization.enabled=true` creates a second Lima VM (`platform-sandbox`, Apple silicon M3+ for nested KVM) on Lima's user-v2 network alongside the k3s VM, hands the chart Lima's SSH identity, and copies the locally built vm image onto the node since there is no registry. `cluster:delete` removes both VMs. Changing an existing install's network is create-time only — recreate the cluster to switch.
