...
Templates with `backend.type: vm` run as smolvm microVMs inside the **sandbox node** — one chart-rendered pod (`<release>-sandbox-node`) built from `packages/controller/Dockerfile.sandbox-node` that holds `/dev/kvm`. Enable it with `virtualization.enabled=true`; the pod requests KVM and tun as device-plugin resources (`virtualization.node.devices`, KubeVirt's `devices.kubevirt.io/kvm` and `/tun` by default, so on OpenShift Virtualization it schedules onto a virt node — give it that node pool's tolerations via `virtualization.node.tolerations`), runs as root with NET_ADMIN, and keeps every machine's disk on one PVC (`virtualization.node.storage`). On OpenShift bind an SCC that admits that (`virtualization.node.scc: privileged` renders the RoleBinding). The node pulls agent images from their registry (private registries need credentials in smolvm's own settings inside the pod; agent pull secrets are not forwarded yet). Locally, `mise run cluster:install -- --set=virtualization.enabled=true` creates the k3s Lima VM with nested virtualization (Apple silicon M3+ on macOS 15+), runs the same image privileged since there is no device plugin there, and copies the locally built vm image archive into the pod since there is no registry. Nested virtualization is create-time only — an existing VM needs `cluster:delete` and a reinstall to gain it.
