...
| Harness | *(proposed, hosted-harness design)* The axis naming where an Agent's agentic loop runs, selected per-template (`harness: pod \| hosted`), copied onto the Agent spec at create and immutable. Orthogonal to Backend (isolation substrate) and Agent Kind (intent marker). The controller ignores it; only the api-server routes on it |
| Pod Harness | *(proposed, hosted-harness design)* Today's model: the harness binary (Claude Code, Codex, …) runs in the agent pod, driven over ACP; sessions are agent-owned. Requires an image shipping the harness-script contract |
| Hosted Harness | *(proposed, hosted-harness design)* The platform's own agentic loop (AI SDK tool loop) running in the api-server, driving the agent pod through agent-runtime's exec surface; conversation history is a Postgres-owned Turn Event Log. LLM credentials resolve through the same Secret Assignments; spend is exported first-party by the api-server |
| Turn Event Log | *(proposed, hosted-harness design)* The append-only per-session event stream (user message, assistant message, tool call, tool result, turn end) that is a Hosted Harness session's source of truth: it drives the LLM context, feeds client streams as a projection, and its tail (no turn-end event) marks a resumable in-flight turn. Events carry `(turn_id, seq)` with a unique constraint — the fencing that makes a stale replica's writes fail closed. A turn runs as a BullMQ job (state-queue pattern); an interrupted tool call resumes as a synthetic failed tool-result and the loop auto-continues |
| Compaction Event | *(proposed, hosted-harness design)* An appended Turn Event Log event carrying an LLM-written summary and the seq range it supersedes. Automatic-only (threshold on estimated prompt tokens before each LLM call). Affects only the LLM context builder (summary + events past the covered range); clients still render the full raw history. The log is never rewritten; a failed summarization fails the turn visibly, never silently truncates |
