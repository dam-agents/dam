...
| Kit Onboarding State | Whether an Agent created from a kit has finished its onboarding. It starts **not fully configured** and becomes complete only when the agent calls the `mark_onboarding_complete` platform MCP tool, which the onboarding prompt instructs it to do once the configuration is genuinely in place; the api-server records the completion time as an Agent annotation. While it is incomplete the scheduler **holds** every schedule on that agent — the occurrence is skipped and the next one queued, never dropped — so an agent that was abandoned half-configured does no work on a cadence. A kit agent whose onboarding is never completed therefore has schedules that never fire, which is deliberate and needs to be visible to its owner |
