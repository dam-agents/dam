...
| Bundled Skill | A skill that ships inside the kit's own definition repository, declared in `kit.yaml` under `bundledSkills` by name and an optional note. **Declaration is for display only**: the apply form lists it so the user sees what the agent will be able to do before creating it, and the platform installs nothing — the harness discovers it as a file once the agent clones the definition. This is the second of two skill states a kit can express, and the separate field is what keeps the Skill Set apply from ever treating one as a dependency |
