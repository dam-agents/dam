...
| Sandbox Node | The non-Kubernetes machine that runs `vm`-Backend Agents as smolvm microVMs, driven by the controller through the sandbox-node machine API (ensure / status / delete per Agent). One per install today |
