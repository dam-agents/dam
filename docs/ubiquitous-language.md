...
| Term | Definition |
|------|-----------|
| Template | A read-only blueprint for a **harness** image the platform delivers — base image, mounts, env, resources, harness family and the model providers it can run on — declared in Helm under `harnessTemplates`. Templates are the harness catalog and nothing else; workload images are starter kits |
...
| Undelivered Prompt | A prompt that never reached the harness — one the Prompt Scheduler dropped, or one a client could not deliver. Its content blocks are kept per Session in the pod's own `undelivered-prompts` document (deliberately not the session-metadata one, which is rewritten on every prompt), reported on every `session/load` without being consumed, and forgotten only when the runtime accepts it again, the user deletes it, or the Session is deleted. Sending it again replays the stored blocks, so pasted images and file references come back with it; inline image bytes past a cap are dropped to names and the message says which. A client that could not reach the pod keeps a text-only copy in browser storage and hands it over on reconnect |
...
| Wrapper Response | A JSON-RPC response frame the API Server publishes when resolving an acp_native row; whichever replica holds the upstream WS for the Agent forwards it to the wrapper |
...
| Size | An Agent's user-facing power: its CPU/memory limits — the template's default at create, else the small chart default of 1 CPU/2Gi, changed afterwards by picking a multiple of a Slot (1x, 2x, 4x) in the Agent's settings. The one resource concept users see — pod requests are scheduling internals derived at render (`max(limit × fraction, floor)`) |
| Slot | The UI's unit for a Budget: one chart-default Size. An Agent takes as many Slots as its larger dimension needs (rounded up); a Ceiling holds as many as its tighter dimension allows (rounded down). Presentation only — enforcement stays in CPU and memory |
...
## Starter Kits (bounded context) — in design (#3576, #3638)

A Starter Kit is a package applied to an Agent — never an Agent, never a Template. Terms below were settled in the format design session of 2026-09-10; the V1 cut is a snapshot at create plus a user-confirmed Kit Pull, with seeding, a read-only definition, work-dir backups and an automatic Update as later increments. Kits are catalog-only platform inputs: the platform never reads configuration back out of an Agent's own repository, so an agent committing to it can widen nothing the platform enforces.

| Term | Definition |
|------|-----------|
| Starter Kit | A package applied to an Agent that seeds what a job needs: workspace configuration files, skills, and the connections, channels and schedules it depends on, plus an onboarding step for what only the user can supply. A kit **may bring its own agent image** — the case for workloads pre-baked into a custom image (the research frameworks): an OCI reference plus the harness family inside it and the providers it runs on, created exactly as a custom-image agent is — in which case the harness choice disappears; otherwise it applies on whichever harness Template the user picks. Templates are the platform's harness catalog and nothing else: a kit is self-sufficient and never needs a Helm entry. A kit without an image may name the harness families it runs on, and the apply form offers only those. It never carries a pull secret: a private kit image is connected at create like any private custom image. **Applying a kit is create-only** (decided 2026-09-10): a kit is instantiated into a new Agent, never applied onto an existing one — a kit may bring its own image, and the definition wants an empty place to land. Applying to an existing Agent, if ever wanted, is a Kit Pull against an Agent with no Kit Version, not a second mechanism. Workspace guidance is seeded as `AGENTS.md`, never a harness-specific file — Claude Code reads it through the `$HOME` symlink experiments already rely on. **Instantiating a kit is a fork** (the intended end state): the definition is taken at the kit's pin and bound to a repository the user owns, where it evolves from then on, and a newer kit reaches it as a pull request into that repository, never as a re-seed. **V1 ships none of the copying** (decided 2026-09-10): the platform seeds no workspace payload at all — the onboarding first turn tells the Agent to clone the kit's `seed` repository itself — and there is no backup repository, no update path and no platform-side sync. The Agent records `<catalog>/<kit>@<version>` and is the user's from the moment it is created. "Preset" is retired (collides with egress presets) and "Template" is taken |
| Kit Catalog | A named index of kits the platform reads. An install reads **several catalogs**, in a fixed order, all through one reader; a kit remembers which catalog it came from. The **built-in catalog** (decided 2026-09-14) is the platform's own — its kit files ship inside the chart and are mounted into the api-server as a directory, so it is versioned with the release, needs no network, and its workload images are pinned by the same publish step that pins every other image. Every other catalog is a git repository URL: a list of repository URLs, each with a pinned ref, seeded per install. A kit is `kit.yaml` at the root of the repository it points at — for an agent definition such as Code Guardian, the definition repository *is* the kit. The catalog is never in the cluster; it is git, read at the pin. **Contributing is one pull request** adding a repo URL and pin; bumping moves the pin. The catalog's CI fetches every entry at its ref and validates it against the Kit Schema, the named Connection Templates and any pinned Template; review judges the rest (run on one harness, honest description, suggested items truly optional, a named maintainer). A video is required for the curated first set, optional for contributions The built-in catalog carries the workload kits, whose images ship with the release; definition kits live in an external catalog, which is seeded by default. An external catalog **moves**: adding a kit to it must not require redeploying the platform, so a periodic refresh job re-reads every catalog and the kits it lists. A catalog entry **may** pin a ref; an entry that does not is **pinned to a commit by the refresh**, so a kit always resolves to an exact commit even when its catalog tracks a branch. The refresh is the only thing that reads the network: it writes what it resolved, and every api-server reads from there |
| Kit Version | The **commit** a kit resolved to, not the ref that named it — a branch or tag is resolved to its sha by the catalog refresh, so two Agents created weeks apart from a moving catalog carry different versions and an Agent's stamp identifies the kit's contents. A kit is identified **namespaced by catalog** (decided 2026-09-14): `<catalog>/<kit>`, with `platform` reserved for the built-in catalog. The Agent remembers `<catalog>/<kit>@<version>`. The version covers everything the kit is **when `kit.yaml` lives in the definition repository** — the typical shape, where `seed` self-references and one commit pins the kit file, `ONBOARDING.md` and the bundled skills together. A kit whose `kit.yaml` sits apart from its definition (the built-in `code-reviewer`) has a second pointer in `seed`, which may be pinned independently or left to float. Built-in kits ship as chart files with no commit to resolve, so their version is the release. Nothing reads the version yet: it is the baseline a later Kit Pull will diff against |
| Kit Pull | *(word open — "Sync" collides with the Live Updates term)* A user-initiated action that diffs the Agent's Kit Version against the catalog's current pin and presents each difference for confirmation — a new or dropped schedule, a new required or suggested connection, a changed env default, a new external skill, the definition version bump. Confirmed platform-half items are applied by the platform; the definition update is handed to the agent as a prompt. Never automatic in V1; "Update" (detection, badge, one button) is the later increment |
| Bundled Skill | A skill that ships inside the kit's own definition repository. `kit.yaml` declares **only where to look** (`bundledSkills.path`, a directory in the seed); the name and description always come from that skill's `SKILL.md` front matter, never copied into the kit file — a copy goes stale the moment the skill is edited. Declaration is for display: the apply form lists them so a user sees what the agent will be able to do, and the platform installs nothing — the harness discovers them as files once the agent clones the definition. The separate field from `skills` is what keeps the Skill Set apply from ever treating one as a dependency. Scanned at the kit's resolved commit, so the result is immutable and cached as such |
| Kit Onboarding State | Whether an Agent created from a kit has finished its onboarding. It starts **not fully configured** and becomes complete only when the agent calls the `mark_onboarding_complete` platform MCP tool, which the onboarding prompt instructs it to do once the configuration is genuinely in place; the api-server records the completion time as an Agent annotation. The tool is registered into an MCP session only while onboarding is pending, so a configured agent carries no tool definition for it. While it is incomplete the scheduler **holds** every schedule on that agent — the occurrence is skipped and the next one queued, never dropped — so an agent that was abandoned half-configured does no work on a cadence. A kit agent whose onboarding is never completed therefore has schedules that never fire, which is deliberate and needs to be visible to its owner |
| External Skill | A skill a kit needs from a repository other than its own, declared in `kit.yaml` by source and name and installed as an ordinary Installed Skill Ref through the Skill Set apply — so its source must be a connected Skill Source, and an entry that cannot apply gets the same closed-set verdict. Skills inside the kit repository are not declared or installed at all — the harness discovers them as files when it loads the definition |
| Connection Requirement | One entry in a kit's `connections`: what it **accepts** — Connection Templates and/or Connection Families — a required-or-suggested flag, and a note shown at apply. A family expands to every template in it; naming a single template is how a kit insists on one method. The apply form shows the requirement with family and template names, filters the user's connections to the expanded set, and offers a Connect button per accepted family or template that opens the connection catalog at that page |
| Connection Family | A named group of Connection Templates the connection catalog presents as one connect page with several methods — `github` (OAuth, PAT, App), `github-enterprise`, `kubernetes`, `modal`, `mcp-server`, `custom-header`. Declared on each Connection Template in the connections catalog and carried on the template view; the catalog grouping and kit requirement checks both read it from there |
| Kit Size | The CPU and memory limits plus the workspace disk a kit's workload needs, declared in `kit.yaml` under `resources` with an optional note explaining why. Stated in limits only — an agent's power and its owner's Ceiling are measured in limits, and requests are derived by the controller, so a kit never states them. Each dimension is independent: a stated one overrides the harness Template's, an omitted one inherits the Template's and then the install default, and the rule is the same whether the kit brings its own image or runs on a picked harness. CPU and memory remain an ordinary Size on the created Agent; disk is fixed at create. This is where the sizing the retired `preconfigured` Templates carried now lives |
| Kit Schedule | A schedule declared in `kit.yaml` with the author's defaults — name, cron or RRULE with timezone, task, `enabled`. The platform creates every one at apply that the user did not **skip** on the apply form; a suggested schedule is created **disabled**, so it is visible on the instance and one toggle away. Onboarding no longer asks for cadence; the user edits the schedule on the instance. Being platform-created is what puts schedules inside the Kit Pull diff |
| Kit Schema | The Zod schema for `kit.yaml` in the contract package, the one source of truth, exported as JSON Schema for the catalog's CI and for editors. `kit.yaml` carries a `schemaVersion`. A kit that fails validation is refused on read with a named verdict, never applied half-parsed |
| Kit Channel | An entry in a kit's `channels`: a messenger type and a note. **Suggested-only** — a channel never blocks apply, because every Agent keeps the chat surface and a Telegram chat can only be bound in-chat. The apply form offers the existing Slack picker with ambient off; other types are a note onboarding repeats |
| Kit Onboarding | The hidden first turn the platform sends a freshly created kit Agent. **Default prompt, platform-composed**: the kit repo and pinned ref, the connections granted by template, the schedules created and which are disabled, the channel bound if any, the declared parameters — then "follow `ONBOARDING.md`". Every kit ships `ONBOARDING.md` beside `kit.yaml`; `onboarding.prompt` in the kit is an optional override, not a prefix. A Kit Pull reuses the same prompt with the new ref and a from–to line |
| Kit Parameter | A value a kit needs that only the user can supply — the repository to act on, the repository that holds the Agent's own definition and state, a label name, a channel roster. Declared in the kit as required or suggested, the same axis as its connections, channels and Template pin; the platform fills what it can detect and onboarding asks for the rest |
