...
| VM Runner | The chart-rendered pod holding `/dev/kvm` that runs `vm`-Backend Agents as smolvm microVMs, driven by the controller through the vm-runner machine API (ensure / status / delete per Agent). One per install today |
