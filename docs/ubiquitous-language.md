...
| Arm | One competitor in an Experiment: a framework plus its Arm Variation. References one existing Agent (the harness, fixed as the Agent's image) and carries an optional Arm Variation. Keyed `(experiment_id, agent_id)`, so the same framework can appear more than once with different variations (identical Arms are allowed — e.g. variance runs). The `connectionGrants` analog |
| Arm Variation | Optional free text on an Arm, appended verbatim to the shared prompt at Trial launch under an `Arm variation:` header — the single declared variable that distinguishes one Arm from another. Opaque to the platform (it stores and forwards a note, never parses it); any structure a harness wants is that harness skill's contract with the user, not a platform-imposed shape. The shared prompt is the un-overridable control; the variation only adds. Empty allowed, no length cap |
...
| record_run | The outbound MCP tool a harness calls to append a Run (Score + `candidate_ref`) to the Run Ledger; attributed to the caller's active Arm by network-verified Agent identity. The autonomous-trial directive in the Trial prompt is what leads a generic harness to call it. Rejected once the calling Arm is no longer `running` (Experiment stopped or Arm already terminal), so the ledger can't be appended after completion or Stop |
| finish_arm | The completion dual of `record_run`: the outbound MCP tool a harness calls once to declare its Arm's loop finished, moving the Arm `running → completed`. Success-only — a harness that gives up just stops and is caught by the Inactivity Deadline; it carries no failure outcome. Prompt-driven like `record_run`, and rejected unless the calling Arm is `running` |
| Arm Status | Per-Arm lifecycle, the source of truth for completion: `pending` (Arm added, Experiment not yet started) → `running` (Trial launched) → one terminal state — `completed` (`finish_arm` called), `failed` (Inactivity Deadline tripped, or the Trial failed to launch), or `stopped` (Experiment Stopped while the Arm was still running). There is no per-Arm `draft`; that is Experiment-level |
...
| KB Template | The installation procedure a Knowledge Base is created from, surfaced to the user as "Template" (distinct from the harness image, which v1 pins and hides). One today — LLM Wiki. The server maps the template id to its Install Command; a new procedure is a new id plus a new mapping |
