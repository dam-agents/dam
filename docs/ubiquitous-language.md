...
| Kit Schedule | A schedule declared in `kit.yaml` with the author's defaults — name, cron or RRULE with timezone, task, `enabled`. The platform creates every one at apply; a suggested schedule is created **disabled**, so it is visible on the instance and one toggle away. Onboarding no longer asks for cadence; the user edits the schedule on the instance. Being platform-created is what puts schedules inside the Kit Pull diff |
