...
| Stage / Span | *(Experiments sense; the OpenTelemetry sense is **Telemetry Span** under Agent Timeline.)* Stage = one declared skeleton node (produce, eval, select, …). Span = one execution of a stage, iteration-keyed, carrying status, timings, an optional numeric Score (captured, plotted, never normalized — the old bet survives), Artifact references, and an opaque `attrs` JSON bag |
| Trace | *(Experiments sense; the OpenTelemetry sense is **Telemetry Trace** under Agent Timeline.)* The append-only stream of spans plus attached Invocations for one Experiment, held by the platform (SDK reports over the per-agent HTTP surface; the browser being closed never pauses a run) |
