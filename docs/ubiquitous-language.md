...
| Contributions Settled | The synthesised readiness gate "the agent has terminated reconciliation for the current desired Version" — true when `runtime_state_outbox.last_settled_version >= version` (or there is no outbox row, i.e. nothing to apply). Does *not* assert per-driver success: a degraded-but-running agent is Settled with non-empty Apply Failures. Distinct from Pod Ready (the K8s condition on the agent pod). `ensureReady` waits on the conjunction `podReady ∧ currentState=running ∧ contributionsSettled ∧ ¬pendingImport`; `computeAgentState` projects the same conjunction to `"running"`. |
| Pending Import | An agent with an incoming file import carries a `agent-platform.ai/pending-import` annotation (value = ISO timestamp); "running" waits while it's fresh. Failure is a *sibling* `agent-platform.ai/import-error` annotation (value = reason, mirroring create-error) — single-purpose keys, no value grammar. The import-proxy re-stamps `pending-import` per attempt, clears both on success, and sets `import-error` on a fatal outcome or the client's give-up — surfaced as the `importError` degraded badge, never wedging "starting" nor silently completing (a failed create clears it too — see Create Error). Roll-robustness is by *reaction*: on a transient failure the proxy returns `503` and the client re-POSTs (idempotent finalize), so any pod roll mid-import self-heals. The `pending-import` TTL is a long (~35 min) backstop for an api-server crash mid-import, not the failure signal. |
| Create Error | The `agent-platform.ai/create-error` marker the api-server stamps (in one atomic write that also clears Pending Import and flips the agent to hibernated) when the create flow fails *after* the Agent ConfigMap exists. Projects the agent to the `error` state ahead of every readiness check, so a half-provisioned agent shows the red Error pill instead of silently reaching `running`. Recovery is delete + recreate. |
