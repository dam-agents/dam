...
| Wake on Finish | A terminal Job outcome waking its Agent with a synthetic prompt carrying what wait would have returned, every field of it — why it ended and what it printed. Always on: an outcome already delivered to a live wait does not wake, and simultaneous finishes coalesce into one turn, up to a bounded count and length with the rest left for the next delivery |
