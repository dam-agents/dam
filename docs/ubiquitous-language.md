...
| Connection Requirement | One entry in a kit's `connections`: the **set of Connection Templates** that satisfy it, a required-or-suggested flag, and a note shown at apply. Naming templates rather than a provider is what lets a kit demand an identity — Code Guardian accepts only the GitHub App and PAT templates, because the reviewer must not be the account that authors the pull requests. The apply form filters the user's connections to the acceptable set and offers to connect a missing one |
