...
| Wake on Finish | A terminal Job outcome waking its Agent with a synthetic prompt carrying what wait would have returned, every field of it — why it ended and what it printed. Always on: a live wait leases the outcome it is blocking on and the wake skips it, so an outcome delivered to a wait does not also wake, and simultaneous finishes coalesce into one turn, up to a bounded count and length with the rest left for the next delivery |
