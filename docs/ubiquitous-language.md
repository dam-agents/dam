...
| Spend Attribution | *(proposed)* An Invocation target has no spend identity of its own: its gateway stamps the **root Driver's** agent id as the trusted telemetry attribution, resolved once at spawn — so a target's tokens and dollars are the Driver's from the moment they are exported, and a Driver's spend is its own plus everything it drove. The mirror of Egress Aliasing for cost, over the same chain and the same root; an unresolvable root fails the create rather than spawning a target whose egress would fail closed anyway. The target's own id rides along as a display-only attribute — the Invocation stays findable in the exploration UI and drillable per-Invocation, but is never a unit of billing. Agent-level only: the target keeps its own Session |
