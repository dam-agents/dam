...
| Surface | Which client an interaction came through (`ui`, `cli`, `other`, or a Channel's own name). Derived once from the authenticated party and stamped on every Activity Event that records an action, not just on the auth fact — the same derivation everywhere, so a turn's surface and a login's surface mean the same thing |
