...
| Precheck | An optional shell command attached to a Schedule, run by agent-runtime in the pod's workspace with the pod's own environment — so credentialed network access through the gateway works — before any Session opens. Its exit code is the verdict: `0` allows the run, `1` declines it, and anything else (a two-minute timeout and a command that will not spawn included) means the Precheck itself broke, which **allows** the run and records the error, so a broken check degrades to the unoptimized behaviour instead of silently stopping work. Its stdout is appended to the task prompt so the run does not re-derive what the check just found; stderr reaches the pod log only. Cron and RRULE Schedules take it alike, and a runtime too old to know the field ignores it and runs — the same fail-open |
