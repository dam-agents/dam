...
| Machine Disk | The single storage disk of a `vm`-Backend Agent's machine, on its owner's VM Runner volume. The vm Backend's whole storage model: the Agent states one size and the guest paths that live on it, rather than the Mounts a pod needs, where each path is a volume with a size of its own. Everything outside those paths is the guest's root overlay, discarded every time the machine stops. Not a Workspace Volume — no PVC of its own, no storage-class pin, no warm pool |
| platform-init | The entrypoint the VM Runner gives every machine: it claims the Machine Disk, applies the mount plan the runner wrote, and execs the image's own entrypoint. It is why a `vm` Agent persists what it declared whatever image it runs — the mounting belongs to the platform, not to the image |
