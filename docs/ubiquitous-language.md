...
| Stage / Span | *(Experiments sense; the OpenTelemetry sense is **Telemetry Span** under Agent Telemetry.)* Stage = one declared skeleton node (produce, eval, select, …). Span = one execution of a stage, iteration-keyed, carrying status, timings, an optional numeric Score (captured, plotted, never normalized — the old bet survives), Artifact references, and an opaque `attrs` JSON bag |
| Trace | *(Experiments sense; the OpenTelemetry sense is **Telemetry Trace** under Agent Telemetry.)* The append-only stream of spans plus attached Invocations for one Experiment, held by the platform (SDK reports over the per-agent HTTP surface; the browser being closed never pauses a run) |
...
## Agent Telemetry (bounded context)

The user-facing read path over the raw signals an agent produced — the structural counterpart to Metrics, which reads the same store for Spend alone. OpenTelemetry's *trace* and *span* mean something different here from the Experiments terms of the same name, so both are qualified. See [`docs/architecture/agent-telemetry.md`](architecture/agent-telemetry.md).

| Term | Definition |
|------|-----------|
| Turn (telemetry) | Everything one exchange produced — the records the harness stamped with one prompt id, and the spans that share their trace or fall inside them. The unit this context lists, chosen over the OpenTelemetry trace because what a trace holds varies exchange to exchange: sometimes a whole exchange, sometimes one model call, sometimes nothing at all. Records the harness did not stamp are grouped on time instead, and each Turn says which rule made it. Addressed by its prompt id within a Session, or by its time range when it has none, so it can hold spans from several traces or from none |
...
| Log Attachment | Where a Telemetry Log Record sits in the trace, and how that was decided: onto the exact call by a request identifier the harness stamps on both the record and the call span; else onto the enclosing span the record names; else onto the trace itself, positioned by timestamp. Which of the three was used travels with the record, because only the first is exact — the record's own span identifier points at the enclosing span, never the model call, while the cost lives only on the record |
