...
| Backend | The isolation substrate an Agent's workload runs on, chosen when the Agent is created and recorded as `spec.backend` — a discriminated union (`type: container \| vm`, default `container`, variant props in a sub-block named after the variant). `container` reconciles the agent StatefulSet (optionally Kata via `runtimeClassName`); `vm` runs the agent as a persistent microVM on the VM Runner, with no pod. A Template never declares one — the same image boots either way. Not "Sandbox" (retired domain term) and distinct from `runtimeClassName`, which selects among *container* runtimes and is rejected on the `vm` backend |
| VM Runner | The pod holding `/dev/kvm` that runs `vm`-Backend Agents as smolvm microVMs, driven by the controller through the vm-runner machine API (ensure / status / delete per Agent). The controller creates one per owner and removes it when that owner has no `vm` Agents left |
...
| Precheck | An optional shell command attached to a Schedule, run by agent-runtime in the pod's workspace with the pod's own environment — so credentialed network access through the gateway works — before any Session opens. Its exit code is the verdict: `0` allows the run, `1` declines it, and anything else (a two-minute timeout and a command that will not spawn included) means the Precheck itself broke, which **allows** the run and records the error, so a broken check degrades to the unoptimized behaviour instead of silently stopping work. Its stdout is appended to the task prompt so the run does not re-derive what the check just found; stderr reaches the pod log, and the tail of it also rides the recorded reason of a broken Precheck out to the owner's panel and the CLI. Cron and RRULE Schedules take it alike, and a runtime too old to know the field ignores it and runs — the same fail-open |
| Declined Fire | A fire that was delivered, woke the Agent, ran its Precheck and got a `1`: no Session opened and no turn was paid for. Distinct from a *suppressed occurrence* (quiet hours — never fires at all) and from a *failed fire* (delivery broke). Counted on the Schedule of its own rather than overwriting the last-run pair, so a quiet Schedule never reads as a broken one — and counted *since the last run*, a run clearing it, so the number answers whether the check is still finding anything rather than accumulating for the life of the Schedule. A broken Precheck is counted separately again, and since the last verdict the script returned rather than since the last run: the run it allowed cannot be what clears it. Editing or removing the Precheck clears every count and reason derived from it, since a record made against one command says nothing about another. A verdict that arrives describing a command the Schedule no longer runs is neither written nor cleared but **dropped**, because clearing would wipe what the new command has already recorded. So recorded state never outlives the field that produced it, and never describes a command that did not produce it. The pod's report also restores the `last-activity` stamp the fire's poke wrote — compare-and-restore, the shape Pause already uses — so a frequent Precheck cannot hold an Agent awake forever and cost more compute than the turns it saved |
...
| Channel Binding | The 1:1 linkage between a conversation surface (Slack channel, Telegram chat) and an Agent; a surface may be bound to at most one Agent globally; the binding itself is the authorization (see Shared Access); Agent delete, Slack disconnect, or Telegram `/unbind` / owner disconnect in the UI releases it |
| Channel Worker | A long-running process that bridges an external service to an Agent |
| Slack Workspace | One Slack workspace the platform can act in, named by Slack's team id. Not the Agent's Workspace (see Substrate) — that is a filesystem. The workspace the operator's Helm bot token was issued for is the **original workspace**, written as the empty string so bindings predating multi-workspace support key identically to later ones |
| Slack Install | One workspace's completed OAuth handshake: the bot token Slack minted for it, held in a Kubernetes Secret, plus the row naming the workspace and who connected it. Slack scopes a bot token to one workspace, so serving several means one Install each; the app-level token stays singular |
...
| Command Pattern | One permitted command shape, written as a usage line. Literals, closed alternations, optional and repeating groups, and Placeholders — deliberately no free-form string form, so a pattern can only ever describe a finite set of possibilities |
| Placeholder | A constrained value slot in a Command Pattern, `<identifier:format>`, where the format is recognized by shape: path glob, integer range, anchored regex, or the default alphanumeric form |
...
| Job | One execution of a Command Pattern match, identified by `(satellite, sequence)` and rendered `gpu-box#7`. Always asynchronous: starting returns the id and the outcome is read back separately. The sequence is minted server-side, since the Satellite has not seen the job when the id must be returned |
| Job Status | `queued`, `running`, `done` (exit code and captured output), `interrupted` (the worker stopped renewing its lease, so the outcome is unknowable), or `cancelled`. Never retried — the commands are not idempotent and nothing can judge one safe to repeat |
| Lease | What a worker renews by heartbeat while running a Job. Expiry is how interruption is detected: a process that dies cannot file a report about itself |
| Work Item | What a worker claims from its long-poll — a newly queued Job, or a cancellation for one it is running. The only direction anything travels toward a Satellite |
| Max Concurrent | How many Jobs a Satellite runs at once — default 16, overridable per Command Pattern for the exclusive ones, clamped to an operator ceiling. The whole bound: non-terminal Jobs may never exceed it, so there is no backlog and `start` either starts or is refused. Parallel by default because `start` returning *running* must be true; an Agent wanting serialization calls wait |
| Job Log | The merged stdout and stderr of a finished Job. Returned inline when small and as a path to a file in the Agent's own sandbox when not, so the Agent reads a large log with its own tools instead of spending context on it. Materialized at read time, when the Agent is definitionally up |
| Wake on Finish | A terminal Job outcome waking its Agent with a synthetic prompt carrying what wait would have returned. Always on: an outcome already delivered to a live wait does not wake, and simultaneous finishes coalesce into one turn |
| Satellite Grant | The per-Agent permission to call a Satellite. The Agent's granted set decides whether the satellite tools are registered at all, and which Satellites its start tool accepts |
