...
| Declined Fire | A fire that was delivered, woke the Agent, ran its Precheck and got a `1`: no Session opened and no turn was paid for. Distinct from a *suppressed occurrence* (quiet hours — never fires at all) and from a *failed fire* (delivery broke). Counted on the Schedule of its own (`declinedCount`, `lastDeclinedAt`) rather than overwriting the last-run pair, so a quiet Schedule never reads as a broken one. The pod's report also restores the `last-activity` stamp the fire's poke wrote — compare-and-restore, the shape Pause already uses — so a frequent Precheck cannot hold an Agent awake forever and cost more compute than the turns it saved |
