...
1. **Trigger kind.** Automatic presses send `trigger: "auto"`, human presses send `"user"`. This
   is what keeps timers out of the activity log, so it must be decided from a real user gesture,
   not from a heuristic.
2. **Pacing.** Refuse an automatic press less than 30 s after the previous one, locally, so the
   server cap is a backstop rather than the everyday path.
3. **Visibility.** Pause automatic presses while the document is hidden and resume on return.
4. **Idle stop.** Stop them entirely after 30 minutes with no human interaction with the page,
   and say so. Any press or interaction starts them again.
...
- [ ] An automatic press sends `trigger: "auto"` and a clicked one sends `"user"`.
- [ ] Automatic presses closer than 30 s apart do not reach the server.
...
chip appears and the presses land no more often than every 30 s. Switch tabs and confirm they
stop, switch back and confirm they resume. Leave it idle past the timeout and confirm it stops
and says why. Check the activity log shows only the presses you made yourself.
