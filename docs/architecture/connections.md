...
Stored contributions are a projection, and a projection can go stale when a premade Template evolves after the Connection was created. Applying an Agent's grants re-renders each granted header-credential Connection's contributions from the Template's current defaults and the stored inputs — the credential is read from the Connection's own Secret, never re-entered — and a changed projection is persisted and delivered over the normal rails. Re-attaching a Connection is therefore the in-place upgrade path; a projection that cannot be re-rendered leaves the stored one untouched, and granting never fails on it.
