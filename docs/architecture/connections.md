...
Two limits follow. An Agent that reaches such a host **without** naming a Connection fails closed at the gateway — the alternative, serving it from whichever credential sorted first, is the silent wrong-account failure this addressing exists to end. And a client that cannot be told a path — `gh`, which takes a host and not a URL — cannot name a Connection, so it reaches a service whose header two granted Connections claim only if one of them is revoked.
