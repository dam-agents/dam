...
- **Connection Requirement** — the *set* of Connection Templates that satisfy it, required or suggested, plus a note. Naming templates rather than a provider is what lets a kit demand an identity: Code Guardian accepts only the GitHub App and PAT templates, because the reviewer must not author the pull requests it reviews.
- **Kit Schedule** — declared with the author's defaults and created by the platform at apply; a suggested one is created **disabled**. Onboarding never asks for cadence; the user edits the schedule on the instance.
