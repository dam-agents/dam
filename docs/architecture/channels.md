...
- **An undecodable attachment is withheld, not forwarded.** An inbound image becomes prompt content only when its bytes are a format the harness decodes — the sniffed type outranks the messenger's label. Anything else is refused with a notice naming the file: a harness handed an image it cannot decode substitutes an internal error, which the agent reports as its answer.
