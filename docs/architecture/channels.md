...
- **An attachment is a picture the agent is shown or a file it is handed.** Which one follows what the sender sent, not what the bytes turn out to be. A picture becomes prompt content only when its bytes are a format the harness decodes, the sniffed type outranking the messenger's label; anything else makes a harness answer with an internal error the agent reports as its own. Everything else is written into the agent's workspace and linked by absolute path, as a web-UI upload is, in whatever format it arrives — a harness opens a PDF with the tools it opens anything with. A file is withheld when the bytes say it never arrived, when it is too big for this process to carry, or when the write fails. What is withheld is named to the agent, so it never answers blind about a file it did not receive; an addressed turn tells the sender too, a read-along turn the agent alone — nobody summoned it.
