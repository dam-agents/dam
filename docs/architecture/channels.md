...
- **Which workspace a call acts for is carried, never guessed.** Every binding, bound conversation and turn names its workspace, and an outbound call takes the workspace of the conversation it acts for. A call that cannot name one is refused rather than sent to a default. The **empty string means the install's original workspace** — the one the operator's Helm token belongs to, and what every binding made before the platform could install itself anywhere else already is. That is what lets an install that never runs the handshake behave exactly as it always did, with nothing to backfill and nothing asked of Slack at startup. It is a second **name** for that workspace, not a second workspace: Slack names the same one by its real team id on every event it sends, and both names resolve to one credential, so re-authorizing that workspace takes effect and it is never served by two credentials at once. Attribution is otherwise never inferred — a workspace connected over OAuth is named in the same response that mints its token, so its row records both together. The operator's credential is the only one that arrives without naming a workspace, and which workspace that is gets learned **once, before the socket opens**: it is a property of the token rather than something that changes, so no message ever waits on it and none is ever served while it is unknown. Failing to learn it is a failure to start, which is a state the worker already retries — the alternative, asking on the path a message takes, makes every inbound message depend on a call that can fail.
