...
- **Resume vs. new is decided by the ACP session list.** The binding lives on the session itself: the worker lists an Agent's sessions over ACP and resumes the one whose thread key matches. A resume that fails (PVC lost, session expired) falls back to a fresh session with history injected — no regression, just a colder start.
- **A resume is caught up on what it missed.** A thread's session belongs to one Agent, so an Agent sees the thread's history only on its first turn there — which left the first Agent to answer in a shared thread blind to every later peer post, answering from a picture frozen at its own last turn. A resuming turn therefore also carries what arrived since that turn, labelled as above and marked unseen; its own posts stay out, and nothing new means no block.
- **`threadKey` is adapter-specific.** Slack pairs the conversation id with the `thread_ts`; Telegram uses the conversation id. Slack's pairing is what isolates an Agent's bindings from each other: a `thread_ts` is only unique within its conversation, so a bare one would cross threads between channels. Nothing in a store guards the key's uniqueness — Slack instead serializes turns per (agent, thread key) across the session match and the prompt: concurrent first messages in a new thread can't mint duplicate sessions, and a mention can't race a second prompt into a session a read-along turn drives.
- **Injected history is attributed per Agent (Slack).** When a fresh session injects thread/channel history, each prior message is labelled by author. Because the single install-wide bot posts for every Agent, a bot message's Slack user id cannot tell Agents apart — so the worker takes the Agent id from each message's [Agent Footer](#the-agent-footer) and labels the line: the reading Agent's own posts become `you (this agent)`, another Agent's posts are named, and humans keep their Slack id. Names are resolved from those ids, one lookup per injected batch, never read off the footer label — so a renamed Agent's older posts carry its **current** name, not the one it posted under. A short legend explaining the prefixes is prepended whenever any appear. A footer-less bot post is named as the bot itself — unattributable, so never the reader's own. Each line carries its send time and an "(edited)" marker.
- **Ambient routing: one session per thread, plus one per channel.** On an ambient binding, a thread reply relays into that thread's own session — the same key a mention there would resume. Top-level messages share a single rolling **ambient session** per channel (a synthetic key in the same slot), so the agent follows the channel rather than starting cold per message. **All** ambient traffic is serialized per session and coalesced: messages arriving while a turn is in flight flush as one multi-message prompt, so a burst avoids the prompt-queue collision `threadKey` describes. A coalesced prompt tags each message with its own ts: a thread batch still answers into its one thread, but a top-level batch has no single triggering message — each batched message registers as its own reply target, and an id-less `reply` refuses rather than threading under the newest. When the agent chimes in, the reply threads under the message it answers; the follow-up conversation runs as an ordinary thread session, with history injection carrying context across.
...
- **Identity-link and binding tables (Postgres).** `identity_links` maps a messenger user to a Keycloak sub, keyed by provider — populated by Slack's `/platform login` (which authorizes the in-chat bind/unbind/ambient/default commands); being provider-keyed, it serves any future workspace channel. Slack bindings live in the channel rows owned by the agents module — one row per bound conversation, so an Agent's several bindings are several rows. The row's identity is its (Agent, conversation) pair — the conversation id being a channel, group DM, or 1:1 DM, undifferentiated — so an Agent cannot connect to one conversation twice, while several Agents may share it. A second, narrower index admits at most one row per conversation marked default — at most, never exactly one, so having no default is a state the schema permits and the routing handles. Each binding carries its own ambient and default flags (absent = off). `telegram_conversations` records the conversation→Agent binding for Telegram (plus the binding owner's sub). Different shapes by design — Slack has a workspace, Telegram does not. Both messengers' tokens live in api-server env from Helm values; there are no channel Secrets in k8s.
