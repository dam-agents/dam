...
- **Identity linking** (person-scoped bindings). A `/platform login` slash command starts a Keycloak OAuth flow; on callback the api-server stores `slack_user_id ↔ keycloak_sub`. All subsequent interactions require a linked identity; unlinked users get an ephemeral prompt to log in. The link table is the source of truth for "who is this Slack user in Platform terms." Shared bindings never consult it. `login`/`logout` stay working but are not listed in the bare-`/platform` usage help (which advertises only `bind`, `unbind`, and `ambient`); the flows that need a linked identity still point users at `/platform login` in context.
...
- **Transport.** Long-poll `getUpdates` — one client for the install, started unconditionally at boot so `/platform bind` works in chats that have no binding yet.
...
- **`threadKey` is adapter-specific.** Slack uses `thread_ts`; Telegram uses the conversation id (chat id + optional forum topic). It is carried on the session as `_meta.platform.threadTs` and matched in-process against the ACP session list; there is no DB uniqueness guard. On Slack the worker instead serializes turns per (agent, thread key) across the session match and the prompt itself: concurrent first messages in a new thread cannot mint duplicate sessions for one key, and a mention cannot race a second prompt into a session a read-along turn is driving — the collision the runtime resolves by silently dropping a queued prompt whose relay connection tears down.
...
- **The agent meets people as ids, and resolves them itself.** A messenger names a person only by their user id — in a shared channel's speaker labels, in injected history, and in the mentions inside message text — so `describe_channel_users` turns a batch of those ids into the people behind them: handle, real and display name, title, pronouns, email, time zone and current status, as far as each person filled them in and the workspace discloses them. Ids resolve one at a time, so an unknown or malformed id costs its own entry rather than the batch, and a looked-up profile (or a confirmed miss) is cached for a few minutes — a channel's regulars are asked about turn after turn. The lookup posts nothing, is gated by the binding like every other outbound affordance, and is recorded in the security log by the ids asked about, never the profiles returned. Slack-only today: Telegram has no workspace directory to consult. Both the per-turn contract and the injected-history legend point at it, so an agent that reads an id knows it can be resolved — unless the tool isn't registered at all, per the scopes model below, in which case neither mentions it.
- **Slack turn tools** — `reply` and `react` — are how a Slack agent answers the turn it is handling. `reply` posts into the current thread; `react` adds an emoji reaction (a Slack short name) to the triggering message — a quiet acknowledgement that notifies no one. A reply may additionally ask to surface in the channel, Slack's native "also send to channel" broadcast: one post, visible both in the thread and in the channel. It exists for threads that have scrolled out of the channel, where a thread-only reply goes unread — and it is off by default, since ordinary back-and-forth belongs inside the thread. The thread and triggering message are named by a `threadTs`/`messageTs` argument that the per-turn prompt injects, so a well-behaved agent always targets the thread it is answering. Omitting the id is a convenience the worker resolves against the turns currently in flight for that Agent: with a single candidate it targets that turn, and outside any candidate it falls back to the Agent's last active thread. Addressed turns (mentions, DMs, fork turns) advance that last-active-thread pointer when they start; ambient read-along turns advance it only through engagement — when a `reply`/`react` actually lands on the turn's thread or message — so a proactive reply never targets a channel message the agent read and silently ignored. A single Agent pod multiplexes every thread over one harness process and one outbound MCP identity, so an id-less call carries nothing to tell concurrent turns apart — when the candidates name more than one target (more than one thread for a `reply`, more than one message for a `react`), the worker therefore refuses the call and asks for the injected id rather than guess and risk posting one thread's reply into another; candidates that agree on the target (say, a retried relay of the same thread) still resolve. The candidates include turns whose relay recently *failed*: a relay that settles with a transport error (connection loss, the turn ceiling) says nothing about the pod — the runtime deliberately keeps a running prompt alive when its relay channel drops — so such a turn may still be executing and calling tools long after the worker saw it fail. Its ref therefore lingers for up to the turn ceiling and keeps counting toward resolution — at worst a refusal answered with the injected id, never a silent cross-thread post. A call that targets a turn — id-less or naming its injected id — posts into that turn's own conversation rather than the one the Agent is bound to at call time, so rebinding an Agent mid-turn cannot redirect an in-flight thread's reply into the newly bound channel. These are Slack-specific and reject when the Agent has no Slack channel connected; Telegram replies stay on `send_channel_message`. `react` needs the `reactions:write` scope.
...
- **Tools are always registered, with one exception.** Calls are rejected at invocation time when no channel is connected for the Agent — no dynamic tool list, no per-session toggle for any binding state; the binding is also the gate for the widened Slack reach. `describe_channel_users` is the one tool gated before that: dropped from the tool list entirely when Slack's granted scopes are confirmed to lack `users:read` (see [Slack scopes: required vs. optional](#slack-scopes-required-vs-optional)), since a missing binding is per-Agent and temporary but a missing scope is install-wide and effectively permanent.
...
Bot scopes in [`deploy/slack-app-manifest.yaml`](../../deploy/slack-app-manifest.yaml) split into two tiers, a distinction worth keeping in mind for any future Slack-facing capability. **Required** scopes back core turn handling (inbound delivery, posting, the working status) and nothing works without them. **Optional** scopes back individual affordances that must degrade rather than take anything else down when the workspace withholds them — today: `channels:read`/`groups:read`/`im:write` (narrow `describe_channel`'s reach), `reactions:write` (the `react` tool), and `users:read`/`users:read.email` (the user directory).

Which of two strategies an optional scope gets follows from what its absence means:

- **Degrade to a smaller result**, when a partial answer is still useful: attempted reactively, with a missing-scope failure caught and turned into a narrower result rather than an outright failure. `describe_channel` without the channel-list scopes still returns the bound channel; a user profile without `users:read.email` still resolves, minus its `email` field.
- **Omit the capability entirely**, when no partial answer is possible: checked proactively instead of per call. The platform asks Slack once what is actually granted and caches the answer for the process's lifetime (a real change means reinstalling the app, which restarts the process anyway); on a confirmed miss, the affordance is left out of the tool list — and out of any prompt text that would otherwise point at it — rather than registered and left to fail forever. `describe_channel_users` takes this path on a missing `users:read`, since no channel could ever make the call succeed. An unreachable bot or an unanswered check counts as _unknown_, never _missing_, and always fails open, so a transient hiccup never hides a capability that in fact works.
