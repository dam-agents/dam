...
Because one install-wide bot serves every Agent, a bare `@bot` cannot say which Agent is meant. So each conversation has exactly one **default Agent** — the first connected, and the one a bare mention reaches — while a mention opening with an Agent's name reaches that Agent. Which Agent holds it can be changed, in-chat. An unrecognised or ambiguous name is not an address: it falls through to the default, which is told the name it could not resolve. Releasing the default leaves the conversation with **none** — nothing is promoted, since that would hand the load to an owner who never accepted it. Unnamed mentions are then refused, naming the connected Agents and how to set one; named mentions still work. A single-Agent conversation therefore behaves exactly as it always did, and nothing about the default surfaces until a second Agent joins.
