...
- **Platform channel** — one app serves the whole install. The operator configures the app once via Helm values; on Slack each workspace adds its own bot token by completing the install handshake, and per-Agent config stays just _which conversation this Agent listens to_. Both messengers are platform channels today. On Slack, identity linking ties messenger users to Keycloak subs at the workspace level.
...
The App-Level Token comes from Helm values and lives in api-server env; a Bot Token is per workspace, comes from the install handshake and lives in a K8s Secret — neither is per-Agent. Resolving the channel's binding yields the Agent and the binding owner; the relay is single-track to the main pod. The workspace-wide identity-link table backs the `/platform login` flow, which authorizes the in-chat bind, unbind, ambient and default commands — never who may drive a turn.
...
- **Token provenance.** The App-Level Token (`xapp-…`) is app-scoped: it comes from Helm values and stays singular, so one Socket Mode connection serves every installed workspace, and it alone decides whether the Slack worker runs. A **Bot Token is per workspace**, because Slack scopes one to one workspace, and every workspace gets its token the same way: by completing Slack's install handshake, the token that mints landing in a K8s Secret with a row pointing at it — no credential in Postgres, and none served from api-server env. Neither token is per-Agent.
- **Token rotation.** Where the Slack app has token rotation turned on, the handshake mints a bot token that expires after twelve hours together with a single-use refresh token, and both go into the workspace's Secret. The token is swapped for a fresh pair once it is an hour old — by a sweep every ten minutes, or by whichever replica needs it first — so an outage has eleven hours to pass before any workspace goes dark, and Slack's two-live-tokens-per-workspace limit is never crossed. The swap runs under the workspace's lock — two replicas spending the same refresh token would leave one holding a revoked pair — with the Secret re-read inside the lock so the later one takes what the earlier one wrote. A refresh that fails while the token still works keeps serving it; once the token has expired, a refresh Slack refuses marks the credential like any other rejection, while one Slack never answered leaves it unmarked for the next try. Slack reports revoked tokens by workspace rather than by token, and a rotated-out token may be the one it reports, so a revocation marks the credential only when the workspace's current token is refused too. Rotation is one switch on the Slack app, but a token minted before it was on keeps not expiring until it is exchanged, one call per token: once the install says rotation is on, every such token is exchanged in one sweep as soon as the lease-holding replica's socket is up, under the same lock. Slack retires the old token as it answers, so the pair replaces it in the workspace's Secret at once. A failed exchange keeps serving the old token and tries again later. Renewing depends only on what a Secret holds, so the install setting only starts exchanges — turning it back off changes nothing for tokens already exchanged, and cannot, since Slack's own switch is one-way.
- **Connecting a workspace is an operator act, and an invitation rather than open enrollment.** A new workspace is install-wide reach, not one Agent's config, so starting the handshake sits behind its own realm role (`platform-slack-installer`) — the chart creates it and a group mapped to it, the same shape the inspector role uses ([usage-tracking](usage-tracking.md)) — and an install that configured no Slack client credentials mounts no install surface at all. What the operator issues is a one-time invitation; the consent itself belongs to an admin of the workspace being added, who needs no platform account. Both sides therefore agree before a workspace is served. A consent redirect the platform did not issue is refused, which is what keeps the app's own distribution URL from being a way in. An install may also name the Slack **organization** it serves, and then a workspace outside it is refused where the credential would be accepted: a conversation id and a person's user id each name one thing only inside one organization, so two organizations at once would let a mention reach the wrong Agent and a person inherit someone else's platform identity. Naming none is the only thing a standalone app can do — Slack reports no organization for one — so the check is off by default rather than refusing every install. Refusing has to withdraw access, not only withhold a record, because by then Slack has installed the app and its events keep arriving: the token Slack minted is revoked, and whether that succeeded is part of what the refusal records, or the record would claim an access that is still live. Nothing else is needed to keep the workspace out — a workspace with no row is served by no credential at all.
- **A binding learns its workspace rather than being told.** The person pasting a conversation id was standing in a workspace when they copied it; the bind surface does not carry that, so Slack is asked instead of the person. Several workspaces answering yes is ordinary — an organization shares one channel into many of them, and each is looking at the same conversation — so **membership decides**: a workspace the bot was invited to can post, one that merely sees the channel cannot, and among equals the workspace connected first wins so repeated binds settle the same way. Only a conversation no connected workspace can see is refused. An in-chat bind skips the question, the slash command having named its workspace; so does a single-workspace install, which answers without asking Slack. A **DM or group DM** can only be bound in-chat once a second workspace exists: the app holds no scope to ask Slack about one, so a DM id pasted into the bind surface is seen by nobody and refused.
- **Which workspace a call acts for is carried, never guessed.** Every binding, bound conversation and turn names its workspace by Slack's team id, and an outbound call takes the workspace of the conversation it acts for. A call that cannot name one is refused rather than sent to a default. Attribution is never inferred — a workspace connected over OAuth is named in the same response that mints its token, so its row records both together.
- **Installs that predate the handshake are migrated, not special-cased.** Such an install was served by a bot token pasted into Helm values, and its bindings name no workspace. While that token is still configured, the lease-holding replica asks Slack which workspace it belongs to **before the socket opens**, makes it that workspace's row — unless the workspace already has one from re-authorizing, whose newer token is kept — and rewrites every binding that names no workspace to name that one, dropping a duplicate the workspace already holds and a second default it already has. It is safe to repeat, so it runs on every boot while the Helm value is set, and the value can be removed once it has run. Doing it before the socket opens means no message is routed while those bindings still name nothing. Slack answering that the Helm token is no good leaves it unimported and those bindings unserved; Slack not answering at all is a failure to start, which the worker already retries.
...
- **Agent persona on posts** is the one optional scope read proactively. With `chat:write.customize`, an agent's own posts (`send_channel_message`, `reply`) carry its name as the username and its avatar as the icon, so a conversation with several Agents shows who said what. Slack does not say whether it refuses or ignores those fields without the scope, so the override is sent only where the granted set is **known** to include it; an unknown set posts under the app's identity, the same message as before. Slack fetches the icon from the public internet and an install has no public URL of its own, so the api-server draws the avatar — the figure the owner sees in the UI, from the owner and the Agent name — to a PNG and uploads it once to ImgBB, named by a hash. A post never waits long on that host: past a short wait it goes out with the name and the app's icon while the upload finishes for later posts, and a failed upload is not retried for several minutes. With no ImgBB key configured the post always keeps the name and the app's icon. Platform notices (a failed turn, a slow start) are the platform speaking and always post as the app.
...
Either way an unreachable bot or unanswered check counts as _unknown_, never _missing_, and fails open, so a transient hiccup never hides a working capability. The agent persona above is the one exception and fails closed: the cost of guessing wrong there is a refused post, while the cost of not guessing is only the app's own name.
