...
- **An attachment is a picture the agent is shown or a file it is handed.** Which one follows what the sender sent, not what the bytes turn out to be. A picture becomes prompt content only when its bytes are a format the harness decodes, the sniffed type outranking the messenger's label; anything else and the agent reports the harness's internal error as its answer. Everything else is written into the agent's workspace and linked by absolute path, as a web-UI upload is, in whatever format it arrives. A file is withheld when the bytes say it never arrived, when the write fails, or when it exceeds what the worker will hold — a per-message ceiling and a whole-worker budget, one process serving every channel, so a burst in one conversation is why another's is refused. What is withheld is named to the agent, so it never answers blind about it; an addressed turn tells the sender too, a read-along turn the agent alone — nobody summoned it.
