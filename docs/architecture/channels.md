...
There is no per-Agent worker registration: a bind is a Postgres row, and every routing path re-reads the binding rows per message, so a bind served on any replica is live everywhere without a signal crossing processes. Bootstrap runs when a replica takes the channel lease: it opens the Slack socket and starts the Telegram client, nothing per Agent. A holder whose transport fails to open does not sit on the lease — the failed acquisition releases it so a healthy replica can win the next campaign.
