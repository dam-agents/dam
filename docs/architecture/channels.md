...
- **A resumed conversation is caught up on what it missed, best-effort (Slack).** A session belongs to one Agent, so it sees a conversation's history only on its first turn there; a mention-only binding is never handed what does not tag it, and an Agent stopped or unreachable is handed nothing at all. A resuming turn therefore also carries what arrived after its own last turn, labelled as above and marked unseen: its own posts stay out, nothing new means no block, and the boundary advances only as far as a read reached. Both a thread's session and the rolling per-channel read-along session are caught up this way. The boundary is in-process, so a restart or a handover loses it; the Agent's own last post stands in, else a rolling recent window — there is always a floor. A capped read says so, and the Agent is told to act only on what is still open. Excluded: Telegram.
