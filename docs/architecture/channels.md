...
- **Reactions are otherwise invisible; `describe_message_reactions` is how an agent asks.** The tool takes a chat (default: the turn's own conversation, else the bound one) and message ts (default: the turn's message), returns each reaction's emoji, count, and reacting user ids, or an error if not found. Never cached, unlike the user directory: a reaction tally is live state — the point of asking (e.g. a scheduled agent checking a signup thread) is the current count. Gated by the binding; the security log records the resolved chat/message and tally, never who reacted. Slack only.
...
- **`no_reply_needed` is cross-channel.** Both messengers instruct the agent to call it to end a turn deliberately silent — a group message not meant for it, or one already handled — rather than leaving undelivered text. It posts nothing and touches no channel, so it is the one turn tool that is not Slack-specific, but it does mark the turn declined — which keeps a sanctioned silence out of the unanswered-turn signal. On Slack it also replaces the old ambient decline token; on Telegram it makes the previously-implicit "just don't reply" an explicit action.
