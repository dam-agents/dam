...
- **Slack mention/fork turns present live, on top of the 👀 ack.** A per-turn *presenter* drives Slack's AI-app surfaces so a working agent looks alive rather than silent: a running status ("is thinking…", the current tool's title as it changes, "is waking…" during a cold start — deduped and throttled, and cleared on every turn-exit path) and the reply *streamed into the thread* (`chat.startStream`/`appendStream`/`stopStream`) as the assistant generates it. Assistant text/thought/tool notifications reach the presenter through a narrow `onUpdate` on the ACP prompt call (raw ACP `session/update` frames, projected to a small worker-owned union; history replayed on resume is withheld). The worker owns the pacing — text is coalesced (flush on a size or time threshold, one append in flight) rather than one API call per token — so the fake gateway sees the same call sequence in tests as Slack does in production. Everything degrades cleanly to the single end-of-turn message: when the event carries no workspace/user id to stream to (Slack requires both in a channel), when the agent streams no text, or when any streaming/status call fails. Streaming applies wherever the agent is summoned — owner, shared, and foreign-fork turns — but never to ambient turns (nobody summoned the agent, and it may decline). This uses only the `chat:write` scope already granted.
