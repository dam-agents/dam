...
Where several Agents read along in one channel, they take each message **one at a time** in a fixed order (longest-connected first), each starting only once the one before it has finished. Running them concurrently would have them talk over each other; running them in order lets each be told whether an earlier one already replied, so a later Agent can add something or stay silent rather than repeat an answer. The cost is deliberate: a channel with several read-along Agents spends that many turns per message, which is why ambient stays a per-Agent opt-in.
