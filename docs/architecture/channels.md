...
- **A binding learns its workspace rather than being told.** A conversation id is unique inside its workspace, so the id someone pastes into the UI or passes to the CLI does not say where it lives. Whichever connected workspace recognises the conversation owns it; an id that several recognise, or none, is refused rather than guessed. An in-chat bind skips the question — the slash command already names its workspace — and a single-workspace install skips it too, answering without asking Slack.
