...
- **Platform channel** — one app serves the whole install. The operator configures the app once via Helm values; on Slack each further workspace adds its own bot token by completing the install handshake, and per-Agent config stays just _which conversation this Agent listens to_. Both messengers are platform channels today. On Slack, identity linking ties messenger users to Keycloak subs at the workspace level.
