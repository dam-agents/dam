...
- **Ambient routing: one session per thread, plus one per channel.** On an ambient binding, a thread reply relays into that thread's own session — the same key a mention there would resume. Top-level channel messages instead share a single rolling **ambient session** per channel (a synthetic key in the same thread-key slot, un-collidable with real thread keys), so the agent genuinely follows the channel rather than starting cold per message. **All** ambient traffic is serialized per session and coalesced: messages that arrive while that session's turn is in flight flush as one multi-message prompt. Each ambient session drains on its own queue — a thread keyed by its `thread_ts`, the channel's top-level flow by its rolling key — so distinct sessions still run concurrently, but a burst within one session never races several turns into it. This matters because concurrent turns on a single session collide in the runtime's per-session prompt queue, which silently drops the losers when their short-lived relay connections tear down — so an un-coalesced thread burst would lose read-along messages with no trace. When the agent chimes in, the reply is threaded under the triggering message; the follow-up conversation in that thread then runs as an ordinary thread session, with thread-history injection carrying the context across.
...
- **Reactions are otherwise invisible, so `describe_message_reactions` is how an agent asks.** Nothing in the message text or injected history says who reacted or with what emoji — the tool takes a chat (defaulting to the bound channel) and a message ts (defaulting to the message the current turn is answering), resolves both the same way the outbound tools do, and returns each reaction's emoji, count, and the user ids who used it — alongside the chat and message it actually resolved to, since the request commonly leaves one or both to default — or an error if the message can't be found. Unlike the user directory this is never cached: a reaction tally is live state, and the whole point of asking — e.g. a scheduled agent checking who signed up on a standing weekly thread — is the current count, not a snapshot from a few minutes ago. Gated by the binding like every other outbound affordance; the security log records the resolved chat and message, and the emoji/count tally, never the reacting user ids. Slack only.
- **Slack turn tools** — `reply` and `react` — are how a Slack agent answers the turn it is handling. `reply` posts into the current thread; `react` adds an emoji reaction (a Slack short name) to the triggering message — a quiet acknowledgement that notifies no one. A reply may additionally ask to surface in the channel, Slack's native "also send to channel" broadcast: one post, visible both in the thread and in the channel. It exists for threads that have scrolled out of the channel, where a thread-only reply goes unread — and it is off by default, since ordinary back-and-forth belongs inside the thread. The thread and triggering message are named by a `threadTs`/`messageTs` argument that the per-turn prompt injects, so a well-behaved agent always targets the thread it is answering. Omitting the id is a convenience the worker resolves against the turns currently in flight for that Agent: with a single live turn it targets that turn, and outside any live turn it falls back to the Agent's last active thread. A single Agent pod multiplexes every thread over one harness process and one outbound MCP identity, so an id-less call carries nothing to tell concurrent turns apart — when more than one turn is in flight at once, the worker therefore refuses the call and asks for the injected id rather than guess and risk posting one thread's reply into another. These are Slack-specific and reject when the Agent has no Slack channel connected; Telegram replies stay on `send_channel_message`. `react` needs the `reactions:write` scope.
