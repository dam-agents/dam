...
- **An undecodable attachment is withheld, not forwarded.** An inbound image becomes prompt content only when its bytes are a format the harness decodes: the sniffed type outranks the messenger's label, and a label claiming no format is still sniffed. A harness handed anything else substitutes an internal error for the picture, which the agent then reports as its answer. What was withheld is named to the sender — separating a format nothing decodes from a download that returned a page instead of the file, which is the shape a missing permission takes — and to the agent in its prompt, so it answers a question about a picture it never received honestly rather than blind.
