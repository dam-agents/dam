...
Slack keeps per-Agent worker registration via `SlackConnected` / `SlackDisconnected` / `AgentDeleted` events on the rxjs bus. That bus is in-process, so the events act only where the workers run — a bind served by another replica reaches the worker through the binding rows, which every path re-reads. Bootstrap runs when a replica takes the channel lease: it starts each transport independently (one outage does not stop the others), then walks the bindings to restore the registrations. A holder with no transport up releases the lease for a healthy replica.
