...
- **Reactions are otherwise invisible; `describe_message_reactions` is how an agent asks.** The tool takes a chat (default: bound channel) and message ts (default: current turn's message), returns each reaction's emoji, count, and reacting user ids plus the chat/message it resolved to (often left to default), or an error if not found. Never cached, unlike the user directory: a reaction tally is live state — the point of asking (e.g. a scheduled agent checking a signup thread) is the current count. Gated by the binding; the security log records the resolved chat/message and tally, never who reacted. Slack only.
- **Slack turn tools** — `reply` and `react` — are how a Slack agent answers the turn it is handling. `reply` posts into the current thread; `react` adds an emoji reaction (a Slack short name) to the triggering message — a quiet acknowledgement that notifies no one. A reply may additionally ask to surface in the channel, Slack's native "also send to channel" broadcast: one post, visible both in the thread and in the channel. It exists for threads that have scrolled out of the channel, where a thread-only reply goes unread — and it is off by default, since ordinary back-and-forth belongs inside the thread. The thread and triggering message are named by a `threadTs`/`messageTs` argument that the per-turn prompt injects, so a well-behaved agent always targets the thread it is answering. Omitting the id is a convenience the worker resolves against the turns currently in flight for that Agent: with a single candidate it targets that turn, and outside any candidate it falls back to the Agent's last active thread. Addressed turns (mentions, DMs, forks) advance that pointer on start; ambient turns advance it only on engagement — a `reply`/`react` landing on the turn's thread or message — so a proactive reply never targets a channel message the agent merely read. A single Agent pod multiplexes every thread over one harness process and one outbound MCP identity, so an id-less call carries nothing to tell concurrent turns apart — when the candidates name more than one target (more than one thread for a `reply`, more than one message for a `react`), the worker therefore refuses the call and asks for the injected id rather than guess and risk posting one thread's reply into another; agreeing candidates (a retried relay of the same thread) still resolve. Candidates include turns whose relay recently *failed*: a transport-error settlement (connection loss, the turn ceiling) says nothing about the pod — the runtime keeps a running prompt alive when its relay drops — so it may still be executing long after the worker saw it fail. Its ref lingers for up to the turn ceiling and keeps counting toward resolution — at worst a refusal answered with the injected id, never a silent cross-thread post. A call targeting a turn — id-less or by injected id — posts into that turn's own conversation, not whatever the Agent is bound to at call time, so a mid-turn rebind can't redirect an in-flight reply into the new channel. These are Slack-specific and reject when the Agent has no Slack channel connected; Telegram replies stay on `send_channel_message`. `react` needs the `reactions:write` scope.
...
Bot scopes in [`deploy/slack-app-manifest.yaml`](../../deploy/slack-app-manifest.yaml) split into two tiers, useful for future Slack-facing capabilities. **Required** scopes back core turn handling (inbound delivery, posting, the working status); nothing works without them. **Optional** scopes back individual affordances that must degrade rather than take anything else down when the workspace withholds them — today: `channels:read`/`groups:read`/`im:write` (narrow `describe_channel`'s reach), `reactions:write` (the `react` tool), `users:read`/`users:read.email` (the user directory), and `reactions:read` (`describe_message_reactions`).
