...
- **An attachment is a picture the agent is shown or a file it is handed.** Which one follows what the sender sent, not what the bytes turn out to be. A picture becomes prompt content only when its bytes are a format the harness decodes (the sniffed type outranks the messenger's label); handed anything else it substitutes an internal error the agent then reports as its answer. Everything else is written into the agent's workspace and linked by absolute path, as a web-UI upload already is — in whatever format it arrives, since a harness opens a PDF with the tools it opens anything with. A file is withheld only when the bytes say it never arrived (markup no such file would hold, or the sign-in page a 200 hides — the shape a missing permission takes) or it is too big for this process to carry. What was withheld is named to the sender, and to the agent in its prompt, so it never answers blind about something it did not receive.
