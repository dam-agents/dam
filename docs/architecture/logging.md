...
The gateway pod runs Envoy, and Envoy is the one component that can neither host an in-process SDK nor be reached by zero-code auto-instrumentation — it is a C++ data plane, not an application runtime. So the gateway's observability is configured **natively in the rendered Envoy bootstrap** (the controller's bootstrap template). The configuration source is the same one every platform service uses (see [observability — platform-service export](observability.md#platform-service-export)): the controller **relays its own inherited `OTEL_*` environment** — set by the chart when the bundled telemetry backend is enabled, or injected by whatever instrumentation a deployment brings — onto each gateway pod, and parses the inherited `OTEL_EXPORTER_OTLP_ENDPOINT` to point the gateway's exporter at the same collector its own SDK exports to. It honors `OTEL_EXPORTER_OTLP_PROTOCOL` (OTLP/gRPC or OTLP/HTTP) and translates `OTEL_TRACES_SAMPLER`/`_ARG` into Envoy's sampling config — Envoy's native tracer reads neither the protocol-driven cluster shape nor those sampler env vars itself, so the controller resolves them at render time. Collector authentication is **transport-level** — the gateway reaches the collector over the ambient mesh (ztunnel mTLS), so no application-layer auth header is configured; the `OTEL_EXPORTER_OTLP_HEADERS` family is deliberately not relayed (Envoy can't consume it, and it may carry a credential). When no endpoint is present, the gateway emits nothing and behaves exactly as an uninstrumented platform. When one is, the gateway emits up to three signals: **traces** (the OpenTelemetry tracer on the outer egress listener), **access logs** (structured JSON on stdout, the same posture as the api-server and controller), and **metrics** (Envoy's stats over an OTLP sink — the admin interface stays disabled, so this push sink is the only stats egress). Metrics are emitted only over OTLP/gRPC (Envoy's stats sink has no HTTP transport), so under the bundled backend's OTLP/HTTP default the gateway emits traces and access logs but no stats; access logs ride the pod log like every other component.

The gateway's own telemetry is **platform telemetry, not agent telemetry**: it exports directly to the collector (a separate path from the agent-telemetry transit chain with its trusted attribution — see [observability — trusted attribution](observability.md#trusted-attribution)), so it carries no `platform.agent.id` and is never attributed to a user. The producing gateway rides along as a bounded `agent.id` resource attribute instead.
...
- **Bounded cardinality.** Every gateway shares one trace/metric `service.name`; per-gateway identity rides as a bounded `agent.id` resource attribute, so cardinality does not scale with the agent count.
