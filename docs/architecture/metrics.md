...
- **Spend breakdown** — the whole Usage tab in one read. Over an absolute half-open `[from, to)` instant range, across *all* of the caller's agents, it returns three rollups together: token/cost **per model**, spend **per agent** (sorted highest cost first, no top-N cap), and spend **per day** for the spend-over-time chart. The range is instants rather than calendar fields on purpose: the client decides what a "month" means in its own timezone, and the server never reasons about calendars. It is a single procedure — rather than one per rollup — so **ownership resolves once per page load** (one owned-agent-list + scope check, not three) and the client renders the tab from a single loading/error state, never showing a stale total above an empty chart while a slower rollup is still in flight.
  - The **per-agent** rollup groups on the trusted, gateway-stamped agent id — the same key every read scopes by; the display name is the latest `platform.agent.name` the telemetry carried in range, read from the rows themselves rather than joined against the live registry, so a since-deleted agent still shows its last known name. The name is display-only; the id remains the sole key and the sole authority for whose spend a row is. Because that id is the *attributed* agent rather than the producing pod (see [Invocation spend](#invocation-spend)), the rollup stays a partition of the window's total no matter how much work an agent delegated: every dollar lands on exactly one bar. The label is taken only from rows the attributed agent produced itself, so a Driver is never renamed after the last child that spent on its behalf; an agent that spent nothing directly in range has no label to take and falls back to its id.
...
An Invocation target is a full Agent with its own pod and its own gateway, so left alone it would carry its own attribution and surface as its own row — a wall of short-lived, machine-named agents beside the Drivers that actually did the work, and a Driver that appears to have spent nothing while delegating heavily. It does not, because a target has no spend identity of its own: its gateway stamps the **root Driver's** id, so the target's spend arrives already attributed to the Driver. See [observability — trusted attribution](observability.md#trusted-attribution) for the stamp and [ubiquitous language](../ubiquitous-language.md) for the term.

The consequence for this page is that **the read path needs to know nothing about Invocations**. There is no lineage join, no recursive walk, no client-side nesting: the rollups group on the attribution id exactly as they always did, and target spend is already inside the Driver's bar by the time the store sees it. This is why the resolution happens at spawn rather than at read — the driver↔target link is a short-lived operational record, not durable history, and a rollup computed at read time would silently orphan a target's spend the moment that record aged out. Resolving once, when the chain is guaranteed present, is what makes spend history immutable in the same way [ownership scoping](#ownership-scoping) keeps a deleted agent's spend from vanishing.

The rollup is **Agent-level only**. A target keeps its own session, so it shows up as its own row in the per-session rollup, under the Driver's agent — the sessions of an agent still sum to that agent's bar. The Overview's trace-aware session fold is a different mechanism (shared trace context between a harness and the runs it spawns in-process) and does not reach across pods into targets: asking what one session cost answers for that session, not for the Invocations it drove.
