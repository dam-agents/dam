...
Four read-only tRPC procedures make up the surface; all are query-only and all are scoped to the caller's own agents. The field-level shapes live in the contract package [`packages/api-server-api/`](../../packages/api-server-api/) — this page describes what each *means*, not its literal type.
...
- **Spend by day** — the same monthly `[from, to)` range read, rolled up into **per-day** cost buckets for a spend-over-time chart. It takes one extra input the other reads don't: an **IANA timezone** string the client supplies (its own `Intl` zone), because a "day" is a wall-clock calendar boundary and the server otherwise never reasons about calendars. The store groups each call into a local calendar day in that zone; the response is **sparse** — only days that actually carried spend appear, keyed `YYYY-MM-DD`. Generating the full month, zero-filling the gaps, and (for the current month) stopping at today are the client's job, the mirror of how **Spend** leaves "what a month is" to the client. Ownership scoping is identical to **Spend**.
