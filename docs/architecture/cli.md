...
- **`dam ssh configure <agent>`** (or **`--all`**) — writes the same managed `Host <alias>` block(s) and `Include` as the editor forms, then prints the alias and example `ssh`/`code`/`zed` invocations and exits without launching anything. `--all` configures every agent on the active host in one pass (writing one `Host` block per agent). For wiring up an editor or other tool by hand.
