...
- **Scripted create requires an explicit model-provider Connection**, addressed by id or unique name. Missing, ambiguous, non-provider, or inactive selections fail before creation, with guidance to choose or connect a provider. The selected Connection is granted in the creation request, using the same server path as the UI.
