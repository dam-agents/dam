...
- **`dam ssh connect -x code <agent>`** (or **`-x zed`**, **`-x pycharm`**) — writes a managed `Host <alias>` block to dam's own `$XDG_CONFIG_HOME/dam/ssh_config` and ensures `~/.ssh/config` pulls it in via one `Include` line, then launches the editor/IDE against the agent's workspace via that alias. Mode `code` runs `<exec> --remote ssh-remote+<alias>` (exec defaulting to `code`); mode `zed` runs `<exec> ssh://agent@<alias>/…`; mode `jetbrains` runs `<exec> jetbrains-gateway://connect#type=ssh&host=<alias>&…` (where `<exec>` is the dedicated `gateway` launcher or any JetBrains IDE such as `pycharm`, which open the same remote-dev link). All resolve the alias from `~/.ssh/config`, so they inherit the same ProxyCommand and identity — JetBrains must be set to use the system OpenSSH config. (`ssh` has no env-var override for its config location — only `Include`/`-F` — and the clients are often already-running singletons that ignore a launch-time `PATH`/`HOME`, so the `Include` is the reliable cross-client hook.)
- **`dam ssh configure <agent>`** (or **`--all`**) — writes the same managed `Host <alias>` block(s) and `Include` as the editor forms, then prints the alias and example `ssh`/`code`/`zed`/`gateway` invocations and exits without launching anything. `--all` configures every agent on the active host in one pass (writing one `Host` block per agent). For wiring up an editor or other tool by hand.
...
`dam ssh` depends on the OpenSSH client (`ssh`, `ssh-keygen`) on the user's machine; the editor modes additionally need the chosen client on PATH (`code`/`code-insiders` for mode `code`, `zed` for mode `zed`, `gateway` for mode `gateway`). Server-side it is image-dependent — agents on images without `sshd` (e.g. distroless) reject the connection.
