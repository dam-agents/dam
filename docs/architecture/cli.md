...
- **`dam ssh <agent>`** (mode `ssh`) — ensures the dam-managed keypair exists, then hands off to the `--exec` client (default `ssh`), configured with a `ProxyCommand` that re-invokes `dam ssh --proxy`. stdio is inherited, so the session is fully interactive.
...
- **`dam ssh -m code <agent>`** (or **`-m zed`**) — writes a managed `Host <alias>` block to dam's own `$XDG_CONFIG_HOME/dam/ssh_config` and ensures `~/.ssh/config` pulls it in via one `Include` line, then launches the editor against the agent's workspace via that alias. Mode `code` runs `<exec> --remote ssh-remote+<alias>` (exec defaulting to `code`); mode `zed` runs `<exec> ssh://agent@<alias>/…`. Both resolve the alias from `~/.ssh/config`, so they inherit the same ProxyCommand and identity. (`ssh` has no env-var override for its config location — only `Include`/`-F` — and the editors are often already-running singletons that ignore a launch-time `PATH`/`HOME`, so the `Include` is the reliable cross-editor hook.)

Authentication is public-key: a dedicated dam keypair under `$XDG_STATE_HOME/dam/ssh/` (so the CLI never touches the user's own `~/.ssh` keys), with a dam-managed `known_hosts` and `StrictHostKeyChecking=accept-new`. The user's real identity is the bearer token verified at the upgrade; the SSH key is only the transport credential `ssh` requires. The interactive and editor forms run the same compatibility gate as other networked verbs; `--proxy` resolves the server from config directly, since it runs once per SSH connection.

`dam ssh` depends on the OpenSSH client (`ssh`, `ssh-keygen`) on the user's machine; the editor modes additionally need the chosen client on PATH (`code`/`code-insiders` for `-m code`, `zed` for `-m zed`). Server-side it is image-dependent — agents on images without `sshd` (e.g. distroless) reject the connection.
