...
- [ADR-058 — SSH access to agents](../adrs/058-ssh-access.md) — `dam ssh` tunnels standard SSH to an in-pod sshd over the agent WebSocket; `--proxy` is the ssh ProxyCommand, `--code` opens VS Code Remote-SSH.
...
- **`dam ssh <agent>`** — ensures the dam-managed keypair exists, then hands off to the system `ssh`, configured with a `ProxyCommand` that re-invokes `dam ssh --proxy`. stdio is inherited, so the session is fully interactive.
- **`dam ssh --proxy <agent>`** — the `ProxyCommand` itself (not run by hand). It resolves the agent, obtains a token, registers the dam public key with the agent via `ssh.authorizeKey` (the agent-runtime tRPC route proxied at `/api/agents/<id>/trpc`), opens a WebSocket to the agent's `/api/ssh` relay, and shovels raw bytes between the socket and stdin/stdout. Only relayed bytes ever reach stdout (the SSH transport); diagnostics go to stderr.
- **`dam ssh --code <agent>`** (and **`--code-insiders`**, **`--zed`**) — writes a managed `Host <alias>` block to dam's own `$XDG_CONFIG_HOME/dam/ssh_config` and ensures `~/.ssh/config` pulls it in via one `Include` line, then launches the editor against the agent's workspace via that alias. VS Code and Insiders use `code`/`code-insiders --remote ssh-remote+<alias>`; Zed uses a `zed ssh://agent@<alias>/…` URL. All three resolve the alias from `~/.ssh/config`, so they inherit the same ProxyCommand and identity. (`ssh` has no env-var override for its config location — only `Include`/`-F` — and the editors are often already-running singletons that ignore a launch-time `PATH`/`HOME`, so the `Include` is the reliable cross-editor hook.)

Authentication is public-key: a dedicated dam keypair under `$XDG_STATE_HOME/dam/ssh/` (so the CLI never touches the user's own `~/.ssh` keys), with a dam-managed `known_hosts` and `StrictHostKeyChecking=accept-new`. The user's real identity is the bearer token verified at the upgrade; the SSH key is only the transport credential `ssh` requires. The interactive and `--code` forms run the same compatibility gate as other networked verbs; `--proxy` resolves the server from config directly, since it runs once per SSH connection.

`dam ssh` depends on the OpenSSH client (`ssh`, `ssh-keygen`) on the user's machine; the editor flags additionally need the corresponding CLI on PATH (`code`, `code-insiders`, or `zed`). Server-side it is image-dependent — agents on images without `sshd` (e.g. distroless) reject the connection.
