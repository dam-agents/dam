...
- **`satellite`** — run this machine's command surface and manage the ones an account has, owned by [satellites.md](satellites.md). `serve` is the one verb with no browser counterpart: it is the worker itself, and its log is how a user reads what the machine accepted, refused and ran.
...
`dam run <agent>` is the CI verb: it speaks ACP to the relay like the UI does — open a Session, submit one prompt, stream the assistant's text to stdout — and exits with a code derived from the turn's stop reason, so a pipeline can branch on the outcome. The prompt comes from a flag or a file (`-` reads stdin — never implicitly, so a promptless invocation errors instead of hanging a CI job). Sessions it creates are typed `cli_run` through the same session metadata every other surface uses — the UI groups them under their own category, and `dam session list` shows the type on each row; every prompt is stamped with a minted prompt identity and the CLI surface marker, so the runtime reports the prompt's fate, frames the turn as a direct one, and records the turn's outcome even when the prompt was sent into a Session another surface created.

The headline property is that **the caller may leave**. Detaching never cancels a running turn, so `--async` exits once the runtime reports the prompt started, printing the session id to poll with `dam run get`. To make the result readable after the fact — the in-memory session log is reaped seconds after an unwatched turn ends, and harnesses differ in what they can replay — the agent-runtime records each finished `cli_run` turn's outcome (prompt identity, stop reason, accumulated assistant text) in a **run record**, a size-capped document on the pod's own disk, deleted with the Session. A runtime-answered extension method serves it back as *done*, *pending* (turn still in flight — including one cut short by an abnormal pod death and awaiting its one automatic resume), *interrupted* (that resume was cut short too, so the runtime will never finish the run — `dam run` reports it as failed with a retry hint), or *none*; a turn the harness dropped mid-run is recorded with no stop reason rather than erased, so a lost run never reads as one that never existed. `dam run get` prints it — under `--json`, the extension method's own status-discriminated payload verbatim, a *pending* shape on wait expiry so stdout is never empty — `--wait` attaches until the turn ends, and `dam run cancel` cancels only a run it could positively read as pending. The record lives in the agent's home volume and shares its lifetime — [persistence](persistence.md) owns what that survives; durability beyond the agent is a deliberate non-goal for now.

The same record is the disconnect story: a `dam run` that loses its socket mid-turn reconnects with backoff, re-loads the Session, and finishes from the record — matching its own turn by the prompt identity and stop reason the logged end-of-turn signal carries — printing only the text it has not already streamed; the total wait is always bounded — `--timeout`, one hour by default (the `agent create --wait` convention) — and on expiry the command exits non-zero with the session id still printed, leaving the run resumable. Permission requests are never answered — a headless auto-deny would also deny the egress approvals the platform injects as synthetic permission requests — the CLI prints a stall line naming the approval and the `dam approval` command that clears it, and the request is already mirrored into the approvals queue where the UI or `dam approval` resolves it out-of-band. That patience has a bound the caller should know: an attached `dam run` keeps the request open until its own timeout, but a detached run (`--async`, or after `run` exits) leaves the request with no engaged channel, and the runtime expires it after its ten-minute orphan window — the tool call fails with an error and the turn runs on to a recorded outcome, so an unattended run stalls for minutes, not forever, at the cost of that one tool call.

## SSH access

`dam ssh connect <agent>` drops into a plain login shell in the agent's workspace and supports everything real SSH does — `scp`, `sftp`, port-forwarding, and editor Remote-SSH. Where `dam chat` attaches to the harness TUI, SSH gives a bare shell. The session inherits the agent pod's environment — the egress proxy, credential sentinels, and `PATH` — so `git`, `gh`, and the AI CLIs work exactly as they do for the harness (the agent-runtime replays the live injected env per connection; without it even DNS fails, since egress is proxy-only). sshd refuses to start a session whose environment exceeds 1000 entries, so the replayed file is capped just under that and sheds Kubernetes service-link vars first — a pod carrying hundreds of them still gets a shell, with its credentials and proxy intact.
...
`dam file get/put/list` are the granular, non-destructive counterparts over the same per-agent file surface the UI's file browser uses, reusing the shared token, compatibility, and resolver seams. Listing can cover one directory or recursively walk the workspace or a subtree, batching directories at each depth within the API’s path-count limit and splitting multi-directory requests by encoded input size to fit the HTTP URL limit. A directory path that exceeds the request budget on its own fails locally before its request is sent. Output names every entry of a listed directory as a workspace-relative path — files and directories alike, with directories distinguished by a trailing separator, so a user can see which subfolders exist to descend into; recursive output is sorted by path. A failed directory read fails the command rather than returning a partial listing as complete. Listing includes dependency directories such as `node_modules`; the server hides only reserved paths and import-staging entries. Import exclusions belong to the import bundle builder. Per-file size caps are enforced server-side; the CLI surfaces the server's too-large error rather than duplicating the constant. Neither verb group introduces a service layer — wire calls are classified inline. Recursive download, large-file streaming, and rm/mv/mkdir are out of scope until a concrete use case appears.
