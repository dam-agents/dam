...
When the server advertises no floor, BelowFloor is never produced. The floor is operator-configurable, so known-broken clients can be dropped without rebuilding the image. Networked verbs opt into the gate; `dam version` is the un-gated informational counterpart that surfaces the same verdict but always exits 0.
...
- **Reserved ID prefix** — the supervisor mints Agent IDs with the `agent-` prefix, and the api-server rejects Agent names that begin with it at create time, eliminating the only ambiguous case.
...
- **Create** is one mutation; env vars and description attach to the payload and the merged Agent is provisioned atomically server-side. **Delete** tears down the sandbox, its gateway, its network namespace and its directory — the same path the UI's delete uses. **Restart** stops the agent's sandbox; the supervisor starts it again on the current spec, and the agent's directory survives.
