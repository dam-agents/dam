...
Everything is tRPC (`connections.list`, `listTemplates`, `create`, `startOAuth`, `discoverMcp`, `get`, `getAgentConnections`, `setAgentConnections`, `delete`) — there is no REST surface and no service state beyond the per-host client. Three pieces of non-obvious behavior:
...
- **`connect` polls a server-side completion marker, not token expiry.** The browser OAuth flow is server-mediated (the api-server is the OAuth client; the provider redirects to the server's fixed callback, which returns to the *web UI* — there is no terminal callback), so the CLI can only observe completion *through the server* by polling `connections.get(id).status`. That status now derives from a `connectedAt` marker the server stamps on every successful token exchange — independent of whether the provider returns a token expiry — so a connection with a non-expiring token (e.g. classic GitHub OAuth) reaches `active` rather than sticking at `pending`. (Before this marker existed, expiry was the only signal, so such connections never read as `active` — in the CLI poll or the UI list.)

MCP servers connect via their URL (`dam connection connect https://…`), which discovers auth and selects the right built-in MCP template — there is no CLI command that lists *connection* templates, so the URL-first form is how MCP servers are added. Listing, granting, revoking, and disconnecting are category-agnostic and unchanged — an MCP connection shows up in `list` with `CATEGORY=mcp` and is removed by `disconnect <id>` like any other.
