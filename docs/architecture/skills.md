...
**Resolving that state.** A publish record also carries `prState`, the last attempt's timestamp, and an ETag, filled in by a periodic job rather than by the `state` query — so GitHub cost tracks the number of unresolved pull requests, not how many users have the Skills page open. Public sources are read **anonymously** from the api-server, which preserves the invariant below: anonymous is not "with credentials", so no credential moves. A private source's pull request is invisible that way, so it is read through a publishing agent's own pod, where the paired gateway injects the owner's token; anonymous is always attempted first, only a `404` escalates, and when several agents published the same pull request each of their pods is tried until a warm one answers. That escalation **never wakes a hibernated agent** — a badge is not worth spending the user's compute unasked — which is acceptable because `merged` and `closed` are terminal: once observed they are persisted and never re-read, so the badge only ever gets more accurate. Cost is held inside the anonymous 60-requests/hour-per-IP ceiling by excluding terminal states from the candidate query and re-checking any one record at most hourly — a record whose attempts keep resolving nothing backs off exponentially to at most daily, so an unresolvable pull request cannot hold an hourly slot of the shared budget forever. Conditional requests are **not** part of that budget: an anonymous `304` is charged exactly like a `200`, so the ETag saves bandwidth only.
...
- The **scan cache** — per-`gitUrl`, 5-minute TTL, invalidated on `sources.refresh` or after a successful publish to that source. Each entry also records the time of the upstream read it came from, and `skills.listWithScan` carries that timestamp so a source card can show how fresh its list is. The cache is empty after a restart, so the value never claims a read that didn't happen.
...
`skills.list(sourceId, agentId?)` returns the bare `Skill[]`; `skills.listWithScan(sourceId, agentId?)` returns `{ skills, scannedAt }` — the same list plus when it was read from upstream, so a source card can render how fresh it is. A cache hit reports the original read; a miss stamps the scan it just performed. Two shapes over one read because the `dam` CLI is installed on users' own machines and outlives any one server: `list` is the array an npm-released CLI pins, so freshness arrives as a sibling procedure rather than by widening it. Both resolve the source and dispatch:
