...
- The **scan cache** — per-`gitUrl`, 5-minute TTL, invalidated on `sources.refresh` or after a successful publish to that source. Each entry records the time of the upstream read it came from, so a source card can show how fresh its list is, and whether that read depended on a user's credentials: an uncredentialed scan is reused for everyone, a credentialed one only for the user whose access produced it. The cache is per-replica and in-memory: the recorded time describes the read that replica performed, and invalidation reaches only that replica.
