...
- The **scan cache** — keyed by the source *and* by the access that produced the scan, 5-minute TTL. An uncredentialed scan is reused for everyone; one that ran under a user's credentials is reused only for that user. Scoping is part of the key rather than a check over a shared slot, so two users of one private source hold separate entries instead of evicting each other — and nothing stands between a lookup and another user's list but the key itself. Expired entries are dropped on the next miss, so the cache holds roughly what was scanned in the last TTL window rather than one entry per user forever. Each entry records the time of the upstream read it came from, so a source card can show how fresh its list is. `sources.refresh` and a successful publish invalidate every scope's entry for that source at once — the upstream moved, which is true whoever read it. The cache is per-replica and in-memory: the recorded time describes the read that replica performed, and invalidation reaches only that replica.
