...
- **MCP `instanceId` is server-bound.** The per-instance MCP endpoint authenticates the agent against the instance ConfigMap status's `platformCredentialHash` ([channels § Auth without an admin login](channels.md#auth-without-an-admin-login), [ADR-039](../adrs/039-platform-credential.md)) and pins the `instanceId` from the verified token, not from tool input.
