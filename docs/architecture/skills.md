...
- **Reconciled `state` view** — joins live `listLocal` from agent-runtime with the `agent_skills` rows, drops ghost rows whose directories were deleted out-of-band (once the outbox is settled; persists the cleanup), and folds in the `agent_skill_publishes` rows.
