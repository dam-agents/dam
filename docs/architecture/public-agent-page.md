...
| **Saga**      | Keeps a row current as the Agent is created, updated, and deleted — a delete writes a tombstone even for an Agent that had no row — and pre-warms it when an Agent is bound to a conversation, so the first click after a bind is already warm. |
