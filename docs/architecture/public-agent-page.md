...
Everything else renders the **generic page at HTTP 200**: an unknown id, a real Agent with no binding, a deleted Agent, and a read that failed are indistinguishable to the reader. A failure answers 200 for the same reason the others do — a 500 on one id and a 200 on every other one is itself an answer about which ids exist — so every path out of this route ends in a page, never an error. Two properties follow, and both are the point:
...
**Public reads never touch the agent record.** An Agent's name lives on its Agent record, so the naive implementation lets anyone with a shell drive control-plane reads at whatever rate they like, on the one endpoint with no principal behind it to rate-limit against.
...
So the page reads an owner-agnostic **Postgres projection** of the Agent's name and owner, held by the Agents context. The binding check gates every read _before_ either the projection or the agent record is consulted, so control-plane reads are bounded by the number of bound Agents rather than by request volume. A miss is only ever paid once per Agent: a view that finds no Agent behind a binding writes a **tombstone** row, so the negative answer is stored too. That matters because a Channel Binding can outlive its Agent — channel cleanup swallows a failed delete and leaves the row — and without a stored negative every view of such an id would reach the control plane again. A control-plane read that _fails_ is not an answer either way, so it writes nothing: the view degrades to the generic page and the next one tries again. Both halves matter. A hiccup that tombstoned would blank a live Agent for good, and a hiccup that escaped as a 500 would make the status code vary with which ids are real, which is the oracle this page is built to deny. The owner's **display name** is resolved from the identity directory for display, and a failed lookup omits the owner line rather than failing the page. It is a name and never the email: the identity directory holds a real mailbox, and this is the one page anyone holding a shared link can read, so an Agent whose owner has no name on record gets no owner line at all.
...
| **Reconcile** | Periodically refreshes rows **that already exist and are still bound**. It does not scan bindings looking for Agents to add — that is lazy fill's job. Its only purpose is catching an api-server that died between the agent write and the Postgres write. |
