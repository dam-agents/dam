...
The idle timeout the checker enforces is **per-agent**, not a single global value: each Agent carries an effective keep-awake timeout — a duration where *never* disables hibernation entirely (the Agent keeps running, and consuming resources, until the value changes) — and falls back to the chart-wide default when it has none. The controller reads only this one effective value, and the idle checker (scale-down) and the reconciler (scale-up) consume it identically so they never disagree; the api-server, the sole writer of the Agent spec, recomputes it on every change. Keep-awake state therefore lives on the Agent resource — observable and reclaimable — not in the pod.

Two inputs determine that value: an operator **baseline** (set by the keep-awake toggle in the UI — *never* keeps the Agent permanently awake; *off* returns it to the default) and one or more **keep-awake pins** that a session-less workload — e.g. a harness-spawned batch job with no chat or terminal attached — takes and releases through the platform's MCP tools. While any pin is held it **governs** the timeout, replacing the baseline (the most-awake pin wins); releasing the last pin restores the baseline, and an explicit baseline change overrides live pins. A held pin replaces rather than raises the baseline, so a workload that pins a finite window can let an otherwise-permanently-awake Agent hibernate once that window elapses — a pin is the workload's own lease, not a floor over the operator's setting. Pins do not expire, so a crashed workload stays pinned until it is purged. The api-server records which input currently governs so a release can't clobber a manual setting, and the UI shows the effective value the controller enforces. Field-level shape lives in [`api-server-api`](../../packages/api-server-api/); time-based pin auto-reclaim is deferred.
