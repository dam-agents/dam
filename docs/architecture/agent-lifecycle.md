...
- **api-server (`last-activity` annotation).** The one traffic-driven signal, and the clock the idle timeout measures against. Bumped (debounced ~30 s) by the chat, terminal, and SSH relays as bytes flow, on a timer while a per-agent tRPC relay is open, by proxied HTTP calls (tRPC and import proxies), by an explicit wake, and by the scheduler on a fire.
