...
When a session goes idle — no engaged channel, no active or queued prompt, no agent-initiated request still pending — the runtime sends `session/close` to the harness. The per-session subprocess is reaped, freeing memory; the transcript stays warm for a time- and byte-bounded window: re-opens stay cheap; the first prompt rehydrates it. Permission requests with no engaged channel time out after ten minutes, answered to the agent as an error so the tool call aborts cleanly.
