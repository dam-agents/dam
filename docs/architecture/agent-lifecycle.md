...
The effective idle timeout is the per-agent `currentHibernationTimeoutMin` (minutes; `0` disables hibernation, so the agent always runs and consumes resources until the value changes), falling back to the chart-wide default when unset. The controller reads only this one field — the idle checker (scale-down) and the reconciler (scale-up) consume it identically, so they never disagree — while the api-server, sole writer of the Agent spec, materializes it on every change. Keep-awake state therefore lives on the CR (observable, reclaimable), not in the pod.

Two inputs feed `current`, and a `currentHibernationTimeoutSource` marker (`manual` | `pins`, default `manual`) records which governs so one path never clobbers the other. The UI writes the operator **baseline** (`baseHibernationTimeoutMin`, a keep-awake toggle: on = `0`/never, off = clear to the default): setting a concrete baseline sets `current` to it and marks the source `manual`, overriding any live pin, while clearing it reverts `current` to the remaining pins (source back to `pins`) — except a still-never revert (a live never-pin) is suppressed to the inherited default with source `manual`, pins left intact, so the operator's off always takes effect (a later acquire takes over again). A session-less workload (e.g. a harness-spawned batch job) instead takes a **keep-awake pin** via the platform MCP tools (`keep_awake_acquire` / `_release` / `_purge`): acquiring marks the source `pins` and sets `current` to the most-awake live pin (a never-pin wins), overriding the baseline. A release recomputes `current` from the remaining pins only while the source is `pins` — under `manual` it just drops the pin — and emptying the set (last release or `purge`) returns the source to `manual` and `current` to the baseline. The UI shows `current`, so the displayed value is what the controller enforces. Pins never expire: a crashed workload stays pinned until an operator purges it (time-based auto-reclaim is deferred, but the marker now carries the provenance a prune would need).
