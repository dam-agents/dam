...
When a session goes idle — no engaged channel, no prompt active or queued, no agent request pending — the runtime sends `session/close` to the harness. The subprocess is reaped, while the transcript stays warm for a bounded window: re-opens stay cheap and the first prompt rehydrates it. Permission requests with no engaged channel time out after ten minutes, answered as an error so the tool call aborts cleanly. A harness that leaves a session load unanswered is wedged: its frames are suppressed (the replay has no request correlation, so a second load is indistinguishable) and the process is recycled as work drains.
