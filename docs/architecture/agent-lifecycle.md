...
Creation is per-purpose: the plain create picks an image and takes the plain path, a [starter kit](starter-kits.md) apply is the entry for anything preconfigured, and the hidden experiment surface creates over the API. Experiment agents are an **Agent Kind** — the flow stamps a marker and dispatches to the owning module rather than the plain agent create, which guarantees a marked agent gets its Install Command. Knowledge bases carry the same marker but are no longer created through it: a knowledge base is a kit that declares one capability, and the agents stamped before kits keep the marker they have. A kit apply is the plain create with the kit's grants and a kit-version annotation, followed by the kit's schedules and channel binding, compensated by delete on failure. A new agent takes its template's size — or, for a starter kit that declares one, the kit's — and the trusted egress preset, both editable on the agent afterwards, plus a Backend the template has no say in: a container, or a microVM, which the API takes beside the image and the browser decides from the owner's own flag rather than asking. What a Template may say about the agent it creates is narrower than it was: the `preconfigured` category is gone, since a preconfigured agent is a kit, and a Template may list the providers its image can run on — a create is refused when that list and the kit's have nothing in common, rather than producing an agent whose harness cannot reach its model. A create may also name a repository to seed into the work directory before the first session (`gitRepo`: URL and, optionally, the branch to stay on); it rides the runtime channel as a `workspace-seed` event ([runtime delivery](runtime-delivery.md#event)) rather than the pod spec, so the pod starts the same way with or without it. See [knowledge-bases](knowledge-bases.md) and [experiments](experiments.md) for what each adds on top of what follows.
