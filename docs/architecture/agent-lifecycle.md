...
- **Satellite-outcome-driven** — a job finishing on a [satellite](satellites.md) commits a `satellite-outcome` event and pokes the Agent, the same shape as a schedule fire. An Agent parked over budget cannot come up, so its outcome is retried on the hour rather than lost.
