...
- **The sender's connection drops while its prompt is still queued.** The runtime cannot answer a channel that is gone, so the sender raises the loss locally, writes the send down in its own storage, and hands that record over when it next reaches the runtime.

The queue itself survives the departure: queued prompts belong to the conversation, not to the channel that sent them, so the last channel leaving **parks** the queue for a grace window, and a client engaging again within it resumes it. A queue leaves the scheduler by four routes — the window expires with nobody back, the session is forgotten, the scheduler clears when the harness goes down, a session the harness cannot load back refuses its queue — and all but the refusal record the queue's prompts as **undelivered** in a durable document on the pod, whole, inline images capped; the refusal errors each sender instead, senders still connected to hear it. Each session load hands the records back, marked on the conversation to send again or delete, and says whether a turn is still running. A record sent again or deleted is **retired**: the load reply names retired records, so the prompt's earlier echo drops from the replay rather than doubling its resend — a note held in process memory only, since a pod restart clears the log that echo lived in too. Recovery is always a send the user initiates: an automatic resend cannot know whether a prompt was dropped before or after the agent saw it, so it risks running it twice. A prompt already *handed to the harness* is the other side of that split: losing the channel then costs the sender its live view and nothing else — the turn runs to completion and its output is appended to the log for any later viewer. Every client that reports delivery honestly depends on this split ([channels](channels.md) states it for its own surface).
