...
Two inputs feed `current`: the operator **baseline** (`baseHibernationTimeoutMin`, written by the UI keep-awake toggle — on = `0`/never, off = clear to the default) and one or more **keep-awake pins** that a session-less workload (e.g. a harness-spawned batch job) takes via the platform MCP tools (`keep_awake_acquire` / `_release` / `_purge`; a never-pin wins, and pins never expire — a crashed workload stays pinned until an operator purges it). The api-server materializes `current` from both on every write and stamps a `currentHibernationTimeoutSource` marker (`manual` | `pins`) recording which one governs, so a pin release never clobbers a manual baseline and the operator's off is never silently overridden by a live pin. The UI shows `current`, so the displayed value is what the controller enforces. (The exact precedence between a fresh baseline write and live pins lives in the keep-awake service; time-based pin auto-reclaim is deferred.)
