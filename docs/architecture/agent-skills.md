...
A **platform skill** — one the platform ships to make a feature usable — is a reading over this verdict, not a fourth value: `system` or `system-modified` **and** a name in the **platform skill registry**, a static skill-name-to-feature map in the contract package ([`platform-skills.ts`](../../packages/api-server-api/src/modules/skills/platform-skills.ts)). Both halves are load-bearing — a name is only a directory on the agent-writable PVC, so a user skill squatting on a registry name never borrows the badge, while origin alone cannot separate a feature's skill from an image's own built-in. Holding the map in code rather than in a marker or a row is what keeps provenance judged against the image. The reading inherits the mechanism's reach: a platform skill is listed whether or not its feature is switched on, because the Skills surface is a truthful view of the Skill Paths — and a skill installed after first boot from outside the image has no pristine counterpart, so it still reads as user-authored.
