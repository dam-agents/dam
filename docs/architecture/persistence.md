...
**The `vm` Backend has no PVCs.** Its workspace is the machine's storage disk on the VM runner pod's volume ([platform-topology](platform-topology.md#vm-runner)), sized from the Agent's persisted mounts and grown in place when that size grows (never shrunk); the guest's boot bind-mounts each persisted path from that disk and everything else on the guest sits on a root overlay that is no persistence contract: it happens to survive a plain stop and start, but any recovery boot discards it. The disk survives hibernation and in-place restarts (the machine is stopped, not destroyed) and goes with the Agent on delete. It is one PVC per owner, shared by that owner's machines rather than held per Agent: the storage-class pin and the warm pool do not apply, and the storage migration never sees it. Besides the Agent's own persisted paths, the disk carries what the platform itself writes there because a machine's root cannot keep it: the guest's boot log, which is where an agent's output goes since a machine's console goes nowhere, kept one boot deep so a machine that died still explains itself on the boot after; and the trust store the boot reuses rather than rebuilding. Both are the platform's, not the Agent's — neither is declared as a mount, and both are rebuilt or resumed by the next boot rather than being state an Agent can lose.
