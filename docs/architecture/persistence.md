...
**The `vm` Backend has no PVCs.** Its workspace is the machine's storage disk on the sandbox node ([platform-topology](platform-topology.md#sandbox-node)), sized from the Agent's persisted mounts and grown in place when that size grows (never shrunk); the guest's boot bind-mounts each persisted path from that disk, plus the docker and k3s state dirs, and everything else on the guest is a throwaway overlay. The disk survives hibernation and in-place restarts (the machine is stopped, not destroyed) and goes with the Agent on delete. It is node-local: it is not on cluster storage, the storage-class pin and the warm pool do not apply, and the storage migration never sees it.
