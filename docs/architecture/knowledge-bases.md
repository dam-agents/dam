...
2. **Install Instruction delivery** over the trigger rail — the same durable outbox event schedules and Experiments use ([connections](connections.md#event-lifecycle), [agent-lifecycle](agent-lifecycle.md#trigger-fire)). The event survives the pod not being up yet, delivers once the agent is Ready, opens a fresh session, and submits the instruction as the prompt. A synthetic per-agent schedule id dedupes redelivery; a TTL bounds how stale an install can land.
3. **Wake**, so the freshly created agent starts installing before the user says anything. The turn runs unattended until the user attaches — the UI lands in the agent's chat immediately after create, so permission prompts have somewhere to go.
...
Knowledge Bases is a feature-gated destination ([features](features.md)) with a list and a single-page create form. Rows open straight into chat — the knowledge base is worked with conversationally, not configured first; lifecycle actions (wake, restart, pause, stop, delete) are the standard agent actions.
