...
The owner's copy in the artifact library is not a second store of record: the edition row is **replaced** by a re-run while the artifact **appends a version** ([artifact-library](artifact-library.md)), so the platform holds only the latest edition while the owner keeps the revision history of their own copy. The skill scopes its lookup of that copy to the artifacts it published itself — every agent an owner runs titles its weekly copy the same way, and an update is owner-scoped rather than agent-scoped, so an unscoped title match would let one agent publish over another's.
