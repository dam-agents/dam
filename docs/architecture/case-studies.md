...
- **The skill** (image-shipped) does the mining, sanitizing, and writing, entirely inside the agent's own sandbox.
...
- **Submission identity is server-stamped.** Agent id from the socket the MCP session arrived on, week start from the server clock, harness image from the agent record; the agent declares only content, window, and its own artifact reference.
