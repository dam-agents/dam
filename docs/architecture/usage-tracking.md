...
- **persist-agents** — writes one `agents` row per `AgentCreated`, marks deleted on `AgentDeleted` (clearing the runtime and config snapshots the row carried); the orphan sweep marks deleted any live row whose Agent no longer exists. A startup bootstrap separately backfills the table from the K8s API for agents that pre-dated the saga.
