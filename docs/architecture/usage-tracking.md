...
- A database-only leak — backup exfiltration, replica compromise, a misconfigured read endpoint — yields opaque pseudonyms. Re-deriving identifiers requires the api-server sandbox or its mounted Secret.
...
- Other surfaces that hold raw subs — K8s ConfigMap `owner` labels, OAuth-connection stored credential keys, `pending_approvals.owner_sub`, `identity_links.keycloak_sub`. Those are out of scope for this subsystem; activity log hardening is the first lever, not the only one.
