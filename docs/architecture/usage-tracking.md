...
A second read surface serves an external usage-analytics pipeline, at the SQL layer rather than over HTTP: one `usage_src_*` passthrough view per table the subsystem reads, each enumerating exactly the columns allowed to leave that table. The views are the privacy boundary — columns holding raw Keycloak subs, and application payloads never written for analytics, are omitted; the activity payload flows through as an object, with each key audited: identity keys are pseudonymized at the write boundary, user-authored identifiers (skill names, source URLs) pass through as the exposed columns already do, and free-form prose (the driver error message) is stripped — and the column list is the contract: a column added to a base table stays invisible until the migration adding it recreates the passthrough, so table migrations are never blocked from outside. Aggregations live with the consumer, which reads nightly through a read-only Postgres role. Privileges in Postgres attach to the object rather than the name, so recreating a view destroys every grant on it — and a view must be recreated rather than replaced whenever a column is renamed or reordered, which is exactly the migration that changes what a consumer reads. Granting the consumer's own login directly therefore revoked its access on precisely the deploys it needed to survive, silently until the next nightly run. So the platform offers a **group role** instead: a credential-less, login-less `usage_readers` that the chart creates and that an operator grants membership in to whichever login should read the metrics. Membership attaches to the two roles rather than to any view, so no migration can break it. The grants themselves stay in the migration history: the migration that creates or recreates a passthrough re-grants it in the same file, naming the views one at a time, so cause and effect sit together and the history records exactly what the consumer can read. That such a step is easy to forget is handled mechanically rather than by discipline — a build gate fails any migration that creates a passthrough without re-granting it. Every grant is guarded on the role existing, so an install with no analytics consumer is unaffected, and the role is absent by default on installs that do not manage their own Postgres. The aggregate views are deliberately withheld from it — a consumer able to read an aggregate would eventually key a metric on one, and renaming that aggregate would break it, which is the coupling this surface exists to remove. Withholding them leaves the passthrough column lists as the only contract, enforced by the grant rather than by review. The role is inert until an operator adds a member, so the install that wants no analytics consumer has nothing to turn off. This is the database-level twin of the inspector role above — the platform offers read-only access over the usage views, and an operator decides who holds it; who that is stays outside the platform's knowledge. The passthroughs are deliberately outside the inspector surface: the `usage_*` aggregate views stay as the backing of the HTML report with no new features, while new metrics are authored consumer-side against the passthroughs.
