...
flowchart LR
  ...
  agent-create --> bus
  boot -.startup agent scan.-> postgres
...
- **`agents`** — Postgres mirror of the agent records. Columns: `id`, `owner_sub` (HMACed), `created_at`, `deleted_at`. Lets SQL views resolve agent ownership without a K8s API round-trip.
...
Every actor identifier written to Postgres is replaced with `HMAC-SHA256(key, value)` rendered as a 64-char hex string. The key — `ACTIVITY_HMAC_KEY` — is a per-install secret generated on first boot and persisted across upgrades.
...
- A database-only leak — backup exfiltration, replica compromise, a misconfigured read endpoint — yields opaque pseudonyms. Re-deriving identifiers requires the api-server process or the node's key file.
...
- An attacker with both the database **and** the api-server sandbox (or its Secret). Pseudonymization is GDPR Recital 26 risk reduction, not anonymization. The stored value remains personal data.
- Other surfaces that hold raw subs — the agent record's `owner`, OAuth-connection stored credential keys, `pending_approvals.owner_sub`, `identity_links.keycloak_sub`. Those are out of scope for this subsystem; activity log hardening is the first lever, not the only one.
...
A weekly retention job runs a bulk DELETE of rows in `activity_events` older than 180 days. It is one of the scheduled per-period jobs described on [platform-topology](platform-topology.md) — Redis-backed, one execution per period, each tick idempotent.
