...
A handler failure leaves the event unsettled, so it is redelivered on the next dispatch until it succeeds or expires; the events behind it in that apply keep running, except behind a failed workspace mutation, which holds them (above). For workspace-mutating kinds the redelivery is bounded: the worker counts each answered-but-unsettled delivery on the event row, and past the attempt budget it stamps the event dispatched-with-error instead of redelivering — the reason the runtime reported stays on the row either way.
