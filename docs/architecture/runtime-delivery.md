...
The call reports the agent's applied cursor (version and hash), its protocol and runtime versions, and its capability set — which contribution and event kinds it can apply, and which optional surfaces its image serves: harness configuration, and the sandbox's own watch surface for live updates. A surface the runtime does not claim is treated as absent, so an older image degrades to the polled path rather than to a broken one; each claim gates both the UI surfaces that read it and the platform's sandbox-facing streams. Because the claim decides membership in those streams, receiving it is itself an Agent change that the platform announces.
...
| `skill-install` | `skill-ref` kind | Materializes a source's skill onto the agent's directory at the pinned version and reaps ones that leave the snapshot. Owned in depth by [skills](skills.md). |
...
1. Driver compares the desired set with what's on disk (or in its own per-kind state file on the agent the agent's directory).
...
Capabilities also gate whole flows, not only payload items: `hello` carries a numeric knowledge-base publish capability, and the api-server delegates share publishing to the sandbox only when it meets the current level — a runtime below it gets the share marked failed with an update-the-agent hint instead of silently never publishing ([knowledge bases](knowledge-bases.md)).
...
| Runtime-state file on the agent the agent's directory | The applied cursor and per-key event last-run timestamps | The timestamps settle redelivered events without re-firing; the cursor answers contribution staleness. |
...
| Per-agent the agent's directory env snapshot file | Reconciled credential-placeholder env | Written by the `env` driver from the channel snapshot (in [`packages/agent-runtime/`](../../packages/agent-runtime/)); read by the harness/terminal spawn paths. |
| `agents` table | Runtime registration per agent (protocol and runtime versions, advertised capabilities, last hello) plus the harness-config snapshot | Registration is rewritten on every `hello`; the snapshot on every apply, and on every sandbox report that changes it. |
| Per-Agent the agent's directory | Materialized files, MCP config, installed skills | Driver-written via runtime channel. |
| Per-driver state file on the agent the agent's directory | Driver's tracking of what it has previously written | Per-contribution-driver, opt-in. Section-marker file driver doesn't need it; key-targeted does. |

## Invariants

- **One agent's delivery never waits on another's.** Every shared stage is either per-agent (the deduplication key, the outbox row lock, the agent-runtime's serialized apply, the connection to the sandbox) or held only for milliseconds (Postgres reads, the job fetch). The only long hold is the HTTP call itself, bounded by its deadline and by one active job per key, and the worker runs far more of those concurrently than agents can occupy. A wedged agent slows only its own deliveries.
...
- **Events fire once per dedupe key and fire time.** The agent's local state store (a per-key last-run timestamp, persisted in the agent's directory) settles redelivered events without re-firing; the worker's `dispatched_at` stamp stops redelivery once acked.
