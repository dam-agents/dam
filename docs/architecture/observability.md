...
The whole stack sits inside the node trust boundary.

## Operators are install-time infrastructure

The telemetry store, its exploration UI and the app-state store behind it run as their own services on the node, brought up with the rest of the stack when the subsystem is enabled. The platform owns only the collector in front of them, which is replaced with an ordinary platform upgrade.

## Access control: the node, not ingestion tokens
...
The telemetry store is a **fourth durable substrate** beyond the three in [persistence](persistence.md) (Postgres, the Agent/Run records, the per-agent directory), and it sits outside that platform/agent split — neither the agent nor the supervisor touches it. Both the telemetry data and the UI's app-state persist on operator-managed volumes that survive sandbox restarts and a chart uninstall; losing those volumes loses telemetry history and nothing else depends on them for correctness. When the subsystem is disabled, none of it exists.
