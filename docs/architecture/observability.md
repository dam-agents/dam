...
- **Trace-context propagation stays on.** The harness keeps W3C trace-context propagation on even when it fronts a custom model upstream — a case where it would otherwise switch it off — so its subprocesses inherit the session's trace context and its requests carry the `traceparent` header. Note the limits: model-request trace context travels inside TLS to the upstream, and the gateway's egress spans do not join harness traces — its tracer sits on the outer listener, which sees only the tunnel (see [logging — gateway telemetry](logging.md#gateway-telemetry)).
