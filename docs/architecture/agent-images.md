...
- **Linux of the image's architecture only.** `mise oci` packages the build host's own tool installs and runs its `apt-get` into a side rootfs, under root. On macOS the task builds in the cluster tasks' Lima VM from a copy of the working tree, with the host's GitHub token for the VM's mise, and copies the result back. The tar is the OCI layout itself, named by its `io.containerd.image.name` annotation for `k3s ctr images import`, with the `manifest.json` of a `docker save` beside it for crane and `docker load`. k-search builds `FROM` that layout as a named build context, not from docker's store.
