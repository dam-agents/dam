...
On both, the [entrypoint](../../packages/agents/base/rootfs/usr/local/bin/agent-entrypoint) trusts the gateway's MITM CA in the system bundle and seeds a new home from the working-dir seed. On a machine it also points docker's client at the gateway, so containers and builds the agent starts go through it too, and puts the share's runtime wrapper in front of docker's and k3s's runc, so they trust the MITM CA as well. A container's root is its own image, which trusts only public roots; the wrapper appends the CA to the bundle that image ships, mounted over it read-only when the container starts, and names it in the common CA variables the container does not set itself. Nothing lands in a built image and no Dockerfile changes. A client with a trust store no variable reaches, like a Java keystore, is not covered. The seed stays pristine: the runtime reconciles image skills against it ([agent-skills](agent-skills.md)).
