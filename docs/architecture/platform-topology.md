...
- **SPIFFE identity per hop.** Agent → gateway, agent → harness, gateway → ext-authz are each gated by a per-instance Istio AuthorizationPolicy keyed on the SA the pair runs as. The waypoint-fronted harness Service enforces principal == URL `:id`; per-instance ext-authz Services enforce principal == matching SA ([ADR-040](../adrs/040-istio-ambient-mesh.md)). No app-layer header conveys identity.
