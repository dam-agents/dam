...
An unsent message is client state too: the composer's text is kept per session in the browser's `localStorage`, so a draft survives a reload or a browser close, belongs to the session it was written for rather than to the composer, and is dropped when it is sent or its session is deleted. Staged attachments live only for the tab — a restored draft names the files it lost.
