...
|------|----------|---------|
| ui → api-server (`<rel>-apiserver`) | tRPC over WebSocket | Resource CRUD, permission flows, and the live-events subscription (server-pushed cache invalidation); the terms flow alone rides tRPC over HTTP, since the socket's admission requires accepted terms |
