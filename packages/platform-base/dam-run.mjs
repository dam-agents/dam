...
// this pod's image, configuration, and RWX /home/agent volume. The executor
// pod dies when this process exits. Derives the harness URL from
// PLATFORM_MCP_URL, opens one WebSocket (/run), and streams stdio via the
// shared frame protocol; interactive invocations get a remote PTY.
//
...
// Allocate a remote PTY only when interactive.
...
// pod (those show up as post-open silence instead).
...
ws.onopen = () => {
  ...
  if (interactive) {
    ...
    // Non-interactive stdin end → EOF. The executor uses a PTY (no true EOF),
    // so signal it the terminal way with EOT — twice, since the first only
    // flushes a partial line (the command reads the 0x04 bytes as data, as in
    // a tty).
...
// Closing the socket tears down the remote side: the Run CR is deleted,
// reaping the executor. In a tty, Ctrl-C reaches the remote PTY as input.
