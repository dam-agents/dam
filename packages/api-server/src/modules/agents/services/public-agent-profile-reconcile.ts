...
export async function reconcilePublicAgentProfiles(
  deps: PublicAgentProfileReconcileDeps,
): Promise<PublicAgentProfileReconcileResult> {
  let agentIds: string[];
  try {
    agentIds = await deps.listProfileIds();
  } catch (err) {
    deps.log(`listing profiles failed, will retry next tick: ${String(err)}`);
