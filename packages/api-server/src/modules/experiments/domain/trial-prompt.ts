...
const AUTONOMOUS_TRIAL_DIRECTIVE =
  "You are running as an autonomous experiment arm, and your work is an iterate-and-score loop: produce a candidate, score it, keep going. No human will reply in this session, so never pause to ask for confirmation, approval, or a go-ahead — make the reasonable call yourself and run the task through to completion, unattended and bounded to a sensible budget. " +
  "Report every scored candidate the moment its score lands, one report per iteration rather than batched at the end, using the record_run and finish_arm tools on the platform-outbound MCP server. For each candidate, write it to a file in your workspace, then call record_run with the iteration's score (a single number, higher is better — negate your metric if it is naturally lower-is-better, such as loss, error, latency, or cost) and candidate set to that file's path (absolute under $HOME or relative to your workspace). Do not delete the file before the call returns; the platform reads it during the call. Each candidate file is capped at 10 MiB. " +
  "When you have no more candidates to produce — the search is exhausted, the budget is spent, or the metric has converged — call finish_arm exactly once, after your final record_run, to mark the arm complete. Attribution is automatic for both tools: the platform resolves your experiment arm from your agent identity, so you never pass an experiment or arm id.";
