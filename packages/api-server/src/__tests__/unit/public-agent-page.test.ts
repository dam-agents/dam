...
describe("public agent page service", () => {
  // TEST_SCENARIO: A stranger pastes an id that was never an agent. The page must
  // answer the same as for a real-but-unbound agent, and must not spend a K8s read
  // on it — the binding check is the cheap gate in front of everything else.
  ...
  // TEST_SCENARIO: An agent exists but nobody connected it to a channel. It is not
  // publicly reachable, so the page treats it exactly like an unknown id.
  ...
  // TEST_SCENARIO: The lazy fill is the only backfill for agents bound before this
  // shipped. The first view writes the row; every later view is served from
  // Postgres, which is the whole point of the projection.
  ...
  // TEST_SCENARIO: The row outlives the agent when a replica dies between the K8s
  // delete and the Postgres write. A view then finds the agent gone, marks the row
  // deleted, and reports the same null as an unknown id.
...
describe("public agent profile saga", () => {
  // TEST_SCENARIO: Neither AgentCreated nor AgentUpdated carries the agent's name,
  // and SlackConnected is the moment the agent becomes publicly reachable. All
  // three therefore read the agent back and write the row, so the first click
  // after a bind is already warm.
  ...
  // TEST_SCENARIO: A deleted agent must stop being named, and a stale link to it
  // has to keep landing on the generic page rather than a 404.
...
describe("public agent profile reconcile", () => {
  // TEST_SCENARIO: The event bus is in-process, so a replica that dies between the
  // K8s write and the Postgres write leaves a stale row behind. The reconcile
  // refreshes renamed agents and retires vanished ones.
  ...
  // TEST_SCENARIO: The reconcile is not a backfill. It walks the rows that exist,
  // never the bindings, so a bound agent with no row stays for the lazy fill to
  // pick up and the two never race.
