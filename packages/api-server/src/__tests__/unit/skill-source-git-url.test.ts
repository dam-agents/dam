...
describe("normalizeGitUrl", () => {
  ...
  // TEST_SCENARIO: the aliases GitHub itself ignores — a www prefix, the default port, letter case — plus a credential, a query and a fragment, which name no repository and must never become part of a stored identity.
  it("folds host aliases and drops what does not identify the repository", () => {
    const forms = [
      "https://www.github.com/DAM-Agents/Dam",
      "https://github.com:443/dam-agents/dam",
      "http://github.com:80/dam-agents/dam",
      "https://user:token@github.com/dam-agents/dam",
      "https://github.com/dam-agents/dam?tab=readme#top",
    ];
    const resolved = forms.map((f) => normalizeGitUrl(f)?.gitUrl);
    expect(resolved).toEqual(
      forms.map(() => "https://github.com/dam-agents/dam"),
  ...
  // TEST_SCENARIO: a directory whose name was percent-encoded in the link becomes a filesystem path, so it must be decoded; a segment that decodes to a traversal names no directory we may scan.
  it("decodes the directory of a browse URL and refuses an unsafe one", () => {
    expect(
      normalizeGitUrl("https://github.com/acme/skills/tree/main/my%20skills"),
    ).toEqual({
    ...
    expect(
      normalizeGitUrl("https://github.com/acme/skills/tree/main/a%2Fb"),
    ).toEqual({ gitUrl: "https://github.com/acme/skills", ref: "main" });
  });

  // TEST_SCENARIO: a branch name holding a slash cannot be told from a directory offline. The split takes the first segment as the branch, so the derived path is a guess the user sees and can correct before the source is created — never a silent one.
  it("splits a slashed branch name by position", () => {
    expect(
      normalizeGitUrl("https://github.com/acme/skills/tree/feat/x/skills/one"),
    ).toEqual({
      gitUrl: "https://github.com/acme/skills",
      path: "x/skills/one",
      ref: "feat",
