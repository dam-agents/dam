...
describe("parseSeedSources", () => {
  ...
  it("rejects malformed JSON with a clear message", () => {
    expect(() => parseSeedSources("{not json")).toThrow(
      /SKILL_SOURCES_SEED is not valid JSON/,
    );
  });
  ...
  it("rejects shape mismatches via Zod", () => {
    ...
    expect(() =>
      parseSeedSources(JSON.stringify([{ name: "x", gitUrl: "not-a-url" }])),
    ).toThrow();
    expect(() =>
      parseSeedSources(
        JSON.stringify([{ name: "", gitUrl: "https://github.com/x/y" }]),
      ),
    ).toThrow();
  });
