...
describe("redactToken", () => {
  it("redacts x-access-token URLs", () => {
    const msg =
      "fatal: https://x-access-token:github_pat_abc@github.com/foo/bar.git not found";
    expect(redactToken(msg)).toBe(
      "fatal: https://[redacted]@github.com/foo/bar.git not found",
    );
  });

  it("leaves clean messages untouched", () => {
    expect(redactToken("nothing to commit, working tree clean")).toBe(
  ...
  it("redacts multiple occurrences", () => {
    ...
    expect(redactToken(msg)).toBe(
      "cloning https://[redacted]@a.com/... and https://[redacted]@b.com/...",
    );
  });
});
