...
const configSchema = z.object({
  ...
  nodeAddress: z.string().default("127.0.0.1:4002"),
  nodeStaleAfterSeconds: z.coerce.number().default(45),
  peerPort: z.coerce.number().default(4002),
  ...
  otelExporterProtocol: z.enum(["grpc", "http/protobuf"]).default("grpc"),
  otelTracesSamplerArg: z.coerce.number().default(0),
...
export function loadConfig(): Config {
  return validatedConfigSchema.parse({
    ...
    nodeId: process.env.DAM_NODE_ID,
    nodeAddress: process.env.DAM_NODE_ADDRESS,
    nodeStaleAfterSeconds: process.env.DAM_NODE_STALE_AFTER_SECONDS,
    peerPort: process.env.DAM_PEER_PORT,
    ...
    otelExporterProtocol: process.env.OTEL_EXPORTER_OTLP_PROTOCOL,
