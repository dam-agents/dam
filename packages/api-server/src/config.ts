...
const configSchema = z.object({
  ...
  /** Directory holding the chart-shipped `repos.yaml` — the curated public-repo
   *  registry an agent's working dir can be seeded from. Boot-loaded like
   *  templates; empty/missing → no repos offered. */
  agentReposPath: z.string().default(""),
...
export function loadConfig(): Config {
  return configSchema.parse({
    ...
    agentReposPath: process.env.AGENT_REPOS_PATH,
