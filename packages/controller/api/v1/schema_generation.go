...
const (
	...
	// AgentStatus — the agent pod's observed restart count, published so the
	// api-server can end an Invocation target that crashed mid-turn without
	// reading pods (which it holds no permission to do).
