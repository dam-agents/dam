...
// new as the schema this controller was built against (ADR-066). Environment
...
// operator-run CRD upgrade in the ops repository (ADR-066).
func Assert(ctx context.Context, client dynamic.Interface) error {
	...
	for name, want := range required {
		...
		if apierrors.IsNotFound(err) {
			return fmt.Errorf("CRD %s is not installed — install the chart CRDs before starting the controller (ADR-066)", name)
		}
		if err != nil {
		...
		if got < want {
			return fmt.Errorf("CRD %s is at schema generation %d but this controller requires %d — run the operator CRD upgrade before deploying this release (ADR-066)", name, got, want)
		}
	}
	return nil
}
