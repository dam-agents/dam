...
func TestBuildEnvoyLeafCertificate_NoSecrets_NotAlwaysIssue_ReturnsNil(t *testing.T) {
	cert := BuildEnvoyLeafCertificate("my-instance", testConfig, configMapOwnerRef(testOwnerCM), nil, nil, false)
...
func TestBuildEnvoyLeafCertificate_NoSecrets_AlwaysIssue_PlaceholderSAN(t *testing.T) {
	cert := BuildEnvoyLeafCertificate("my-instance", testConfig, configMapOwnerRef(testOwnerCM), nil, nil, true)
...
func TestBuildEnvoyLeafCertificate_L7HostsExtendSANs(t *testing.T) {
	...
	// terminates TLS for it — including for a fork with no credential
	// Secrets (alwaysIssue=false must now issue).
	cert := BuildEnvoyLeafCertificate("my-instance", testConfig, configMapOwnerRef(testOwnerCM), nil, []string{"api.github.com"}, false)
...
func TestBuildEnvoyLeafCertificate_DedupesAndSortsHosts(t *testing.T) {
	...
	cert := BuildEnvoyLeafCertificate("my-instance", testConfig, configMapOwnerRef(testOwnerCM), secrets, nil, false)
	require.NotNil(t, cert)
...
func TestBuildEnvoyLeafCertificate_TelemetryHostInSAN_ZeroSecretsFork(t *testing.T) {
	...
	cert := BuildEnvoyLeafCertificate("my-instance", &cfg, configMapOwnerRef(testOwnerCM), nil, nil, false)
...
func TestBuildEnvoyLeafCertificate_TelemetryHostDedupedWithChainHost(t *testing.T) {
	...
	cert := BuildEnvoyLeafCertificate("my-instance", &cfg, configMapOwnerRef(testOwnerCM), secrets, nil, false)
	require.NotNil(t, cert)
