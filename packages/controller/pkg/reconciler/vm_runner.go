...
// UNIT_BOUNDARY_DESCRIPTION: smolvm can give each machine's VMM its own unprivileged uid, and this runner turns that off, because it cannot have it and the shared unpacked image at the same time. A dropped VMM reaches the image store through an idmapped mount of one entry, on-disk uid 0, so every file the image gives another uid arrives in the guest as nobody — 27,374 of this image's 35,430, whose workload then exits the moment it starts. Measured both ways on one store: with the drop the guest boots in 150 ms and dies; without it the same tree presents those files as the user the image named, and the machine runs. Machines whose rootfs comes from the older per-machine archive fail to finish starting under the drop as well, by a route not traced here — so this is the mechanism that was isolated, not the whole of what the drop costs.
// UNIT_BOUNDARY_DESCRIPTION: the runner no longer unpacks an image, so CHOWN and FOWNER — held for a tar that had to restore an image's own ownership — now guard nothing, and are kept only until something proves they are unused. DAC_OVERRIDE still earns its place: a release that briefly gave each machine's VMM its own uid chowned those machines' directories to it, and the runner, uid 0 and no longer the owner, is permission-checked like anyone else on the directories it must go on managing. Dropping it would strand every machine created while that was on.
