-- Satellites: owner-scoped command surfaces on machines outside the cluster, the
-- per-agent grants that reach them, and the jobs they run. A job carries its own
-- lease so a worker that dies is detected rather than trusted to report.
