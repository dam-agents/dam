...
-- identity_links gains team_id for the same reason but keeps its primary key:
-- inside one enterprise Slack organization a user id already identifies one
-- person, and widening the key would make anyone who ran the login command in
-- one workspace run it again in the next.
...
ALTER TABLE "identity_links" ADD COLUMN "team_id" text DEFAULT '' NOT NULL;--> statement-breakpoint
