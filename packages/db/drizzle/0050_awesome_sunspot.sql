...
-- Secret and the row only points at it, so Postgres holds no credential. Slack
-- names the workspace in the same response that mints the token, so a row
-- records both together and which workspace a credential serves is never
-- inferred. credential_state is an enum rather than free text because a value
-- outside the known set would read as usable and quietly serve a dead
-- credential.
-- The install that predates this table has no row at all: it keeps being served
-- by the operator-supplied token in api-server env. That token answers for the
-- one workspace it was issued for, so a workspace with no row is served by
-- nothing.
--
