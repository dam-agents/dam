...
- **Table change**: edit `src/schema.ts` → `mise run //packages/db:generate` (writes the `.sql`, `_journal.json` entry, and snapshot — never hand-edit them) → add a top comment explaining *why*.
- **View change**: `mise run //packages/db:new -- <name>` scaffolds the `.sql` + journal entry, then hand-write the `CREATE/DROP VIEW` SQL (dependency order; `--> statement-breakpoint` between statements).
- Never hand-write a table migration: `mise run //packages/db:check:generated` (part of `mise run check`, no database) fails if the snapshot doesn't match `schema.ts`.
