...
async function processTrigger(
  ...
  try {
    ...
    if (config.PLATFORM_MCP_URL) {
      // No Authorization header here — Envoy in the paired gateway pod
      // attaches it on the wire (issue #108).
...
/**
 * POST to the api-server's `/internal/trigger` endpoint via `fetch` — undici
 * honors HTTP_PROXY/NODE_USE_ENV_PROXY, so the request flows through the
 * paired gateway pod where Envoy attaches the per-instance platform
 * credential before forwarding (issue #108).
 */
async function postTrigger(
  ...
  if (!res.ok) {
    throw new Error(`POST /internal/trigger failed: ${res.status} ${text}`);
  }
  try {
