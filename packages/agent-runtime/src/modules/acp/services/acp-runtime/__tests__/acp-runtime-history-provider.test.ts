...
describe("acp-runtime: session-history provider", () => {
   ...
   * error instead of waiting forever, and the session must stay cold so the
   * next prompt starts a fresh rehydrate.
   */
  it("should time out a hanging rehydrate, fail held prompts, and stay cold", async () => {
    const world = createWorld({
      historyProvider: providerOf(["m1"].map(updateLine)),
      rehydrateTimeoutMs: 1,
    });
    ...
    bob.send(frames.prompt(3, SESSION, "retry"));
    expect(world.harness().received("session/load")).toHaveLength(2);
  });
