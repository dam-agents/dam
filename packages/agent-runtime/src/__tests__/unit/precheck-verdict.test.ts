...
describe("precheck verdict", () => {
  ...
  // TEST_SCENARIO: 127 and 126 are the check breaking, not saying no, so the run goes ahead and the reason survives.
  ...
  // TEST_SCENARIO: what the cheap check printed rides along, so the expensive turn does not re-derive it.
  ...
  // TEST_SCENARIO: a task that says "see the precheck output below" must never be followed by nothing, so a botched exit code still hands over what it printed.
