...
// UNIT_BOUNDARY_DESCRIPTION: whether a reference is one a cache entry can be named after. Hand-written to the pattern `^[a-zA-Z0-9][a-zA-Z0-9._/:@-]{0,254}$`. The `..` a caller might hide in a reference is refused separately, by the reader, because this pattern admits the dots a tag legitimately carries.
...
// UNIT_BOUNDARY_DESCRIPTION: the caller must hold a lock across this, and this function cannot check that it does. Reading which ports are taken and writing the chosen one are two steps, so two allocations that interleave choose the same free port and both write it — two machines published on one port, and the second one unreachable. The server holds its ports lock around the whole call, and nothing else calls it.
// UNIT_BOUNDARY_DESCRIPTION: the machine's directory has to exist already — `ensure` writes the share first, and that is what creates it. Creating it here instead would let a caller that never set the machine up leave a directory behind that `machine_ids` reads as a machine and the allocator counts, holding a port for something that does not exist.
