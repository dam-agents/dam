...
```sh
mise run //packages/agents/nous:image          # plain docker build (pip-installs nous from GitHub)
...
```sh
NOUS_REF=main mise run //packages/agents/nous:image     # track a branch
NOUS_REF=v0.4.1 mise run //packages/agents/nous:image   # or a different tag
```
