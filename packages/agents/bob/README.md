...
| `BOB_TELEMETRY_*` | The platform's export rail, not free-form: `BOB_TELEMETRY_PROVIDER`, `_URL`, `_SERVICE_PATH` and `_AGENT_OPS_ENABLED` are set for the agent when the telemetry backend is enabled, pointing Bob's OTLP exporter at the platform collector through the ordinary gateway egress. Overriding them by hand redirects the agent's spend telemetry, so leave them to the rail. Bob 2.0.3 also reads the standard `OTEL_EXPORTER_OTLP_*` variables and lets any of them override the provider, accepting only `http/json`, so those stay unset as well. The exporter ignores `HTTPS_PROXY`, so on the platform the export does not reach the collector today. |
