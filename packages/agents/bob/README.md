...
The image is built with [`mise oci`](https://mise.jdx.dev/dev-tools/mise-oci.html) from [`packages/agents/base`](../base/), as its `bob` config environment ([`image.toml`](image.toml)). Its files live at their image paths under [`rootfs/`](rootfs/).
...
MCP servers still arrive the platform way, as a runtime-channel contribution written to `~/.bob/settings/mcp.json` (see [`runtime-manifest.yaml`](rootfs/app/runtime-manifest.yaml)), not through `session/new.mcpServers`.
