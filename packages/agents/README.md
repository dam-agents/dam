...
| `nous/`, `openevolve/`, `shinkaevolve/`, `gepa/`, `skydiscover/` | `mise oci` | Workloads: an `image.toml` environment over claude-code's (`-E claude-code,<workload>`), its Python package a `pipx:` tool with its venv in `$<NAME>_VENV`, and `rootfs/`. |
| `k-search/` | Dockerfile | A workload `FROM` the claude-code image, with its own harness and runtime manifest: it clones and patches two repositories. |
