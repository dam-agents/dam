...
/**
 * Library-style picker for adding a connection. Scrollable inside a
 * fixed-height modal — searchable across every OAuth app the api-server
 * knows about, plus the two manual paths (MCP server, custom secret).
 * OAuth apps lay out in a 2-column grid to keep the list compact; manual
 * options stack below as wider rows.
 */
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-2xl h-[80vh] flex flex-col gap-4">
        <DialogHeader className="shrink-0">
          ...
          <DialogDescription>
            Pick what to connect — credentials are encrypted in the cluster
            and never visible to the agent runtime.
          </DialogDescription>
        </DialogHeader>
