...
export function ScheduleSessionRow({ session, onResume }: Props) {
  ...
  return (
    ...
    >
      <span className="text-[12px] text-foreground font-medium truncate flex-1">{displayName}</span>
      <span className="text-[10px] text-muted-foreground shrink-0">created at: {createdAt}</span>
    </div>
  );
}
