...
describe("applyUpdate — the harness's name for the prompt", () => {
  test("platform_turn_ended stamps the harness prompt id on the reply it closes", () => {
    const start: Message[] = [
      userMsg("u1", "hi"),
      assistantMsg("a1", "hello", true),
    ];
    const out = applyUpdate(start, {
      sessionUpdate: "platform_turn_ended",
      sessionId: "test-sid",
      telemetryPromptId: "otel-1",
    });
    expect(out[1].streaming).toBe(false);
    expect(out[1].telemetryPromptId).toBe("otel-1");
  });
...
describe("applyUpdate — the late telemetry follow-up", () => {
  test("platform_turn_telemetry stamps the last settled reply that has no id", () => {
    const start: Message[] = [
      userMsg("u1", "first"),
      assistantMsg("a1", "one", false),
      userMsg("u2", "second"),
      assistantMsg("a2", "two", false),
    ];
    const out = applyUpdate(start, {
      sessionUpdate: "platform_turn_telemetry",
      sessionId: "test-sid",
      telemetryPromptId: "otel-late",
    });
    expect(out[3].telemetryPromptId).toBe("otel-late");
    expect(out[1].telemetryPromptId).toBeUndefined();
  });

  test("platform_turn_telemetry skips a reply that already carries an id", () => {
    const start: Message[] = [
      userMsg("u1", "first"),
      { ...assistantMsg("a1", "one", false), telemetryPromptId: "otel-1" },
    ];
    const out = applyUpdate(start, {
      sessionUpdate: "platform_turn_telemetry",
      sessionId: "test-sid",
  ...
  test("platform_turn_telemetry does not stamp a still-streaming reply", () => {
    const start: Message[] = [
      userMsg("u1", "first"),
      assistantMsg("a1", "typing", true),
    ];
    const out = applyUpdate(start, {
      sessionUpdate: "platform_turn_telemetry",
      sessionId: "test-sid",
