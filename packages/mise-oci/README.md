...
- **A second entrypoint around the shared one.** [`debian-entrypoint`](debian-base/rootfs/usr/local/bin/debian-entrypoint) runs before [`agent-entrypoint`](../platform-base/entrypoint.sh), to remap `agent` to root in extrausers and create sshd's `/run/sshd` on a root machine. It runs after it too, to sync the system CA bundle when the trust store came from the vm Backend's cache.
