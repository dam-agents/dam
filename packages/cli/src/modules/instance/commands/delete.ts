...
export function buildDeleteCommand(deps: {
  ...
  return new Command("delete")
    ...
    .action(async (ref: string, opts: { server?: string; yes?: boolean; json?: boolean }) => {
...
async function runDelete(ref: string, opts: { server?: string; yes?: boolean; json?: boolean }, deps: DeleteDeps): Promise<void> {
  ...
  if (!result.ok) {
    if (result.error.kind === "not-found") { alreadyGone = true; }
    else { printServiceError(result.error, host); process.exit(EXIT_INSTANCE_RUNTIME_FAILURE); }
  }
