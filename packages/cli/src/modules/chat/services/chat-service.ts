...
export function createChatService(deps: {
  ...
  return {
    async listSessions(input) {
      ...
      if (!result.ok) return err({ kind: "session-failed" as const, reason: result.error.reason });
      return result;
    },
    ...
    async run(input) {
      ...
      let resolution = await sessions.resolveTerminal(instanceId, input.strategy, { reset: input.reset });
      if (!resolution.ok) return err({ kind: "session-failed" as const, reason: resolution.error.reason });

      if (resolution.value.kind === "confirm-mode-switch") {
        if (!await deps.confirmModeSwitch()) return err({ kind: "mode-switch-declined" });
        resolution = await sessions.resolveTerminal(instanceId, { kind: "resume", sessionId: resolution.value.sessionId }, { reset: input.reset, force: true });
        if (!resolution.ok) return err({ kind: "session-failed" as const, reason: resolution.error.reason });
      }

      const r = resolution.value;
      if (r.kind === "confirm-mode-switch") return err({ kind: "session-failed" as const, reason: "unexpected mode-switch prompt" });
