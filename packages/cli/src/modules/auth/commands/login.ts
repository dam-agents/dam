...
export function buildLoginCommand(deps: LoginCommandDeps): Command {
  return new Command("login")
    .description("Authenticate against a Platform deployment via OAuth 2.0 Device Authorization Grant")
    .option("--server <url>", "Platform server URL — persisted as the active host")
    .option("--no-browser", "Do not auto-open the browser; print the verification URL instead")
    .option("--force", "Skip the re-login confirm prompt when already logged in")
    .action(async (opts: { server?: string; browser?: boolean; force?: boolean }) => {
