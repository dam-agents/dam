...
describe("sshPaths", () => {
  // Locks the XDG empty-string-is-unset rule and the fixed file names the
  // ProxyCommand and known_hosts pinning depend on.
  it("honors XDG_STATE_HOME", () => {
    ...
    expect(p.knownHosts).toBe("/tmp/xdg-state/dam/ssh/known_hosts");
  });
